import { buildMonthArchive } from './hr-month-archive.js?v=42h191';
import { registerPwaUpdateGuard } from './pwa.js?v=42h191';
export function createMonthArchiveUI(root,adapter){
  ...
  const unregisterUpdateGuard=registerPwaUpdateGuard(()=>busy?{safe:false,reason:'הפקת ארכיון הדוחות עדיין מתבצעת.'}:{safe:true});
  ...
  return {destroy(){dead=true;unregisterUpdateGuard();clear();unsubscribe();month.removeEventListener('change',clear);window.removeEventListener('pagehide',clear);button.remove();status.remove();note.remove();}};
}
