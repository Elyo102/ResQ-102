...
const mutations = [
  {
    ...
    probe:"const s=require('./schedule-person-contract'); let ok=false; try{s.normalizeSchedulePerson({schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:'uid_1',display_name:'x',active:true,revision:1,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:'x'}}});}catch(e){ok=e.code==='external-link-forbidden';} if(!ok)process.exit(1);"
  },
  {
    name:'link requires current revision', file:'schedule-person-service.js',
    from:'if (person.revision !== expected)', to:'if (false)',
    probe:"const s=require('./schedule-person-service'); let ok=false; try{s.planLink({person:{schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:null,display_name:'x',active:true,revision:4,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:'x'}}},expected_revision:3,actor_uid:'a',actor:{uid:'a',station_id:'eilat_102',authorized:true},uid:'u',user:{exists:true,active:true,station_id:'eilat_102',uid:'u'},station_link:null,global_link:null});}catch(e){ok=e.code==='link-stale';} if(!ok)process.exit(1);"
  },
  {
    name:'inactive schedule person cannot be linked', file:'schedule-person-service.js',
    from:'if (person.active !== true)', to:'if (false)',
    probe:"const s=require('./schedule-person-service'); const p={schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:null,display_name:'x',active:false,revision:1,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:'x'}}}; let ok=false; try{s.planLink({person:p,expected_revision:1,actor_uid:'a',actor:{uid:'a',station_id:'eilat_102',authorized:true},uid:'u',user:{exists:true,active:true,station_id:'eilat_102',uid:'u'},station_link:null,global_link:null});}catch(e){ok=e.code==='link-inactive';} if(!ok)process.exit(1);"
  },
  {
    name:'person revision remains safely incrementable', file:'schedule-person-contract.js',
    from:'!Number.isSafeInteger(revision) || revision < 1 || revision >= Number.MAX_SAFE_INTEGER',
    to:'!Number.isSafeInteger(revision) || revision < 1',
    probe:"const s=require('./schedule-person-contract'); let ok=false; try{s.normalizeSchedulePerson({schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:null,display_name:'x',active:true,revision:Number.MAX_SAFE_INTEGER,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:'x'}}});}catch(e){ok=e.code==='person-revision';} if(!ok)process.exit(1);"
  },
  {
    ...
    probe:"const s=require('./schedule-recipient-contract'); const p={schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'registered',linked_uid:'uid_1',display_name:'x',active:false,revision:1,source_ref:null}; if(s.recipientsForChanges({station_id:'eilat_102',changed_person_ids:[p.person_id],people:[p],verified_users:[{uid:'uid_1',active:true,station_id:'eilat_102'}]}).length!==0)process.exit(1);"
  },
  {
  ...
  },
  {
    ...
    probe:"const s=require('./schedule-person-service'); let ok=false; try{s.planLink({person:{schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:null,display_name:'x',active:true,revision:1,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:'x'}}},expected_revision:1,actor_uid:'a',actor:{uid:'a',station_id:'eilat_102',authorized:false},uid:'u',user:{exists:true,active:true,station_id:'eilat_102',uid:'u'},station_link:null,global_link:null});}catch(e){ok=e.code==='actor-forbidden';} if(!ok)process.exit(1);"
  },
  {
    name:'uid namespace cannot collide with person ids', file:'schedule-person-contract.js',
    from:"!uid.startsWith('sp_')", to:"true",
    probe:"const s=require('./schedule-person-contract'); let ok=false; try{s.normalizeSchedulePerson({schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'registered',linked_uid:'sp_account_001',display_name:'x',active:true,revision:1,source_ref:null});}catch(e){ok=e.code==='linked-uid';} if(!ok)process.exit(1);"
  },
  {
    ...
    probe:"const s=require('./schedule-recipient-contract'); const p={schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'registered',linked_uid:'uid_1',display_name:'x',active:true,revision:1,source_ref:null}; if(s.recipientsForChanges({station_id:'eilat_102',changed_person_ids:[p.person_id],people:[p],verified_users:[{uid:'uid_1',active:false,station_id:'eilat_102'}]}).length!==0)process.exit(1);"
  },
  {
  ...
  },
  {
    ...
    probe:"const s=require('./schedule-import-identity'); const p={schema_version:1,person_id:'sp_registered_001',station_id:'eilat_102',kind:'registered',linked_uid:'u',display_name:'Canonical',active:true,revision:2,source_ref:null}; const b={station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'employee',value:'1'},person_id:p.person_id,expected_person_revision:2}; const r=s.planImportIdentities({station_id:'eilat_102',source_namespace:'station-workbook-v1',entries:[{source_key:b.source_key,display_name:'Alias'}],inventory:[p],bindings:[b]}); if(!r.ready||r.assignments[0]?.person.display_name!=='Canonical')process.exit(1);"
  },
  {
  ...
  },
  {
    ...
    probe:"const s=require('./schedule-import-identity'); const p={schema_version:1,person_id:'sp_registered_001',station_id:'eilat_102',kind:'registered',linked_uid:'secret',display_name:'x',active:true,revision:1,source_ref:null}; const b={station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'employee',value:'1'},person_id:p.person_id,expected_person_revision:1}; const r=s.planImportIdentities({station_id:'eilat_102',source_namespace:'station-workbook-v1',entries:[{source_key:b.source_key,display_name:'x'}],inventory:[p],bindings:[b]}); if(Object.hasOwn(r.assignments[0].person,'linked_uid'))process.exit(1);"
  },
  {
  ...
  {
    name:'name source references are NFC canonical', file:'schedule-person-contract.js',
    from:"sourceValueRaw.normalize('NFC')", to:'sourceValueRaw',
    probe:"const s=require('./schedule-person-contract'); const d='Jose\\u0301'.normalize('NFD'); const p=s.normalizeSchedulePerson({schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'external',linked_uid:null,display_name:'x',active:true,revision:1,source_ref:{station_id:'eilat_102',source_namespace:'station-workbook-v1',source_key:{kind:'name',value:d}}}); if(p.source_ref.source_key.value!==d.normalize('NFC'))process.exit(1);"
  },
  {
  ...
  },
  {
    ...
    probe:"const s=require('./schedule-identity-store-contract'); const p=s.publicPerson({schema_version:1,person_id:'sp_person_001',station_id:'eilat_102',kind:'registered',linked_uid:'secret',display_name:'x',active:true,revision:1,source_ref:null}); if(['linked_uid','kind','linked'].some(k=>Object.hasOwn(p,k)))process.exit(1);"
  },
  {
...
for (const mutation of mutations) {
  ...
  const baseline = spawnSync(process.execPath, ['-e', mutation.probe], { cwd:dir, encoding:'utf8' });
  assert.equal(baseline.status, 0,
    mutation.name + ': probe fails on the correct implementation\n' + baseline.stdout + '\n' + baseline.stderr);
