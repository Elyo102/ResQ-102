 ...
 *  ----------------------------------------------------------------
 *
 *  „שינוי אחד בסידור שלך" הוא נכון, עובר כל בדיקת מבנה — ואינו
 *  אומר את הדבר היחיד שההתראה נשלחה בשבילו: **מה השתנה**.
 *  בדיקה שסופרת שדות במטען לא הייתה תופסת את זה לעולם, כי המטען
 *  היה תקין לחלוטין.
 *
 *  ⭐ לכן הבדיקה הזאת משווה **מחרוזות**. היא שואלת שאלה אחת: אם
 *  הזיזו אותי, ההודעה אומרת לאיזה יום ולאיזו תחנה?
 *
...
if (moved) {
  // ⭐ „הוזזת", ועם המקום שממנו: „הוזזת לתמנע" לבדו מחייב לפתוח
  // את האפליקציה כדי להבין אם זה בכלל שינוי מבחינתך.
  eq('2.2 העברה בין תחנות · מאיפה ולאן',
    moved.push.body, 'הוזזת · 4/9 · מראשית לתמנע');
...
if (removed) {
  ...
  // ברור על איזה שיבוץ מדובר. ⭐ ואין „ביטול": שיבוץ אינו מבוטל,
  // והעובדה התפעולית היא שביום הזה אינך משובץ.
  eq('2.7 ירידה משיבוץ', removed.push.body, 'אינך משובץ · 4/9 · ראשית');
}

// ⭐ המקרה של אלדד: „אני בראשית כל השבוע והזיזו אותי ליום אחר".
const movedDay = notify(
  plan([row('2026-09-04', 'rashit', 'ראשית', [slot('דן', 'driver', 'נהג')])]),
  plan([row('2026-09-06', 'rashit', 'ראשית', [slot('דן', 'driver', 'נהג')])]),
  'דן');
if (movedDay) {
  eq('2.7a העברה ליום אחר היא שינוי אחד',
    movedDay.push.body, 'הוזזת · מ-4/9 ל-6/9 · ראשית');
  eq('2.7b ופריט אחד בלבד', movedDay.push.items.length, 1);
}
const movedBoth = notify(
  plan([row('2026-09-04', 'rashit', 'ראשית', [slot('דן', 'driver', 'נהג')])]),
  plan([row('2026-09-06', 'timna', 'תמנע', [slot('דן', 'driver', 'נהג')])]),
  'דן');
if (movedBoth) {
  eq('2.7c יום אחר וגם תחנה אחרת',
    movedBoth.push.body, 'הוזזת · מ-4/9 ל-6/9 · מראשית לתמנע');
}
