...
function validateIndexes(value) {
  assert.deepEqual(Object.keys(value).sort(), ['fieldOverrides', 'indexes']);
  assert.deepEqual(ordered(value.indexes.filter(i => domainGroups.includes(i.collectionGroup))), ordered(expectedIndexes));
  // Domain records are expired by the bounded dispatcher using expires_at_ms;
  // they do not write an expires_at Timestamp or claim a native TTL policy.
  assert.deepEqual(value.fieldOverrides.filter(i => domainGroups.includes(i.collectionGroup)), []);
}
await check('exact25 HR domain indexes and retention policy are independent of unrelated product indexes', () => validateIndexes(config));
await check('in-memory omitted, changed-scope and additional HR domain indexes fail the closed gate', () => {
  for (const mutate of [value => value.indexes.splice(value.indexes.findIndex(i => domainGroups.includes(i.collectionGroup)), 1),
    ...
    value => value.indexes.push({ collectionGroup: intentGroup, queryScope: 'COLLECTION_GROUP', fields: [] })]) {
    const bad = structuredClone(config); mutate(bad); assert.throws(() => validateIndexes(bad));
  }
  const unrelated = structuredClone(config);
  unrelated.indexes.push({ collectionGroup: 'unrelated_product', queryScope: 'COLLECTION_GROUP', fields: [] });
  unrelated.fieldOverrides.push({ collectionGroup: 'unrelated_product', fieldPath: 'expires_at', ttl: true, indexes: [] });
  validateIndexes(unrelated);
});
