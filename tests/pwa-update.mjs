...
async function scenario(kind, activate = true, updateFails = false) {
  ...
  const result = await refreshInstalledApp({
    version: '42F.3', serviceWorker: sw, cacheStorage, location,
...
for (const kind of ['waiting', 'installing', 'active']) {
  ...
  assert.deepEqual(got.deleted, ['resq-v41e-release1', 'resq-v42f1-release1', 'resq-v42f2-release1'], kind + ': only old ResQ caches are deleted');
  assert.equal(got.replaced.length, 1, kind + ': reload runs once');
  assert.match(got.replaced[0], /updated=42F\.3-12345/, kind + ': reload URL is fresh');
  if (got.worker) {
