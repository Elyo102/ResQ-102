...
check('metrics.html follows admin.html head conventions (rtl, viewport, theme.css?v=42h29, one App Check init)', /<html lang="he" dir="rtl">/.test(page) && /<meta name="viewport"/.test(page)
  && /href="\.\/theme\.css\?v=42h29"/.test(page) && (page.match(/await initAppCheck\(app\);/g) || []).length === 1);
check('every local module import on metrics.html carries ?v=42h29', [...page.matchAll(/from '\.\/([^']+)'/g)].every((m) => /\?v=42h29$/.test(m[1])) && [...page.matchAll(/from '\.\/([^']+)'/g)].length >= 6);
...
check('page calls only getMetricsDashboard through monitored-functions', /from '\.\/monitored-functions\.js\?v=42h29'/.test(page) && /call\('getMetricsDashboard', \{ days \}\)/.test(read('metrics-ui.js')) && !/recordMetrics/.test(page));
