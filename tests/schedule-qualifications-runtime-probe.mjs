...

{
  ...
  eq('2.2 ניסיון חוזר → duplicate, אותה קבלה ובלי שינוי', [again.duplicate, again.revision, db._get(ST + '/schedule_qualifications/diver').revision], [true, 1, 1]);
  ...
  const clearedU3 = await rt.setPersonQualifications(req({ request_id: 'h4', person: 'u3', qualifications: [], expected_revision: 1 }));
  eq('4.4a ניקוי משאיר tombstone עם revision עולה',
    [clearedU3.revision, db._get(ST + '/schedule_person_qualifications/u3').qualifications,
      db._get(ST + '/schedule_person_qualifications/u3').revision,
      db._get(ST + '/schedule_person_qualifications/u3').cleared],
    [2, [], 2, true]);
  const clearedU3Again = await rt.setPersonQualifications(req({ request_id: 'h4', person: 'u3', qualifications: [], expected_revision: 1 }));
  eq('4.4b ניסיון חוזר לניקוי מחזיר את הקבלה המקורית',
    [clearedU3Again.duplicate, clearedU3Again.qualifications, clearedU3Again.revision],
    [true, [], 2]);
  ...
  const clearedU2 = await rt.setPersonQualifications(req({ request_id: 'h-clear-u2', person: 'u2', qualifications: [], expected_revision: 3 }));
  eq('4.9 ניקוי אינו מאפס CAS', [clearedU2.revision, db._get(ST + '/schedule_person_qualifications/u2').revision], [4, 4]);
  await rt.setPersonQualifications(req({ request_id: 'h-after-clear-u2', person: 'u2', qualifications: ['driver'], expected_revision: 4 }));
  const oldClearReplay = await rt.setPersonQualifications(req({ request_id: 'h-clear-u2', person: 'u2', qualifications: [], expected_revision: 3 }));
  eq('4.10 ניסיון חוזר ישן מחזיר קבלת ניקוי מקורית ואינו דורס שינוי מאוחר',
    [oldClearReplay.duplicate, oldClearReplay.qualifications, oldClearReplay.revision,
      db._get(ST + '/schedule_person_qualifications/u2').qualifications,
      db._get(ST + '/schedule_person_qualifications/u2').revision],
    [true, [], 4, ['driver'], 5]);
  await rejectsCode('4.11 לקוח ישן עם revision 0 נדחה אחרי הקצאה וניקוי (אין ABA)',
    () => rt.setPersonQualifications(req({ request_id: 'h-aba', person: 'u2', qualifications: ['driver'], expected_revision: 0 })),
    'holdings-revision-stale');

  await rt.saveQualification(req({ request_id: 'q-rope', key: 'rope', label: 'חילוץ חבלים' }));
  await rt.setPersonQualifications(req({ request_id: 'h-rope', person: 'u8', qualifications: ['rope'] }));
  db._del(ST + '/users/u8');
  await rejectsCode('4.12 אי אפשר להקצות כשירות לעובד שעזב',
    () => rt.setPersonQualifications(req({ request_id: 'h-rope-assign', person: 'u8', qualifications: ['driver'], expected_revision: 1 })),
    'person-not-member');
  const departedClear = await rt.setPersonQualifications(req({ request_id: 'h-rope-clear', person: 'u8', qualifications: [], expected_revision: 1 }));
  eq('4.13 אפשר לנקות בלבד החזקות קיימות של עובד שעזב',
    [departedClear.qualifications, departedClear.revision, db._get(ST + '/schedule_person_qualifications/u8').cleared],
    [[], 2, true]);
  const departedReplay = await rt.setPersonQualifications(req({ request_id: 'h-rope-clear', person: 'u8', qualifications: [], expected_revision: 1 }));
  eq('4.14 ניסיון חוזר לניקוי עובד שעזב מחזיר אותה קבלה',
    [departedReplay.duplicate, departedReplay.revision], [true, 2]);
  await rejectsCode('4.15 ניקוי מזהה שעזב בלי רשומת החזקות אינו יוצר tombstone',
    () => rt.setPersonQualifications(req({ request_id: 'h-ghost-clear', person: 'ghost', qualifications: [] })),
    'person-not-member');
  const ropeDeleted = await rt.deleteQualification(req({ request_id: 'd-rope', key: 'rope', expected_revision: 1 }));
  eq('4.16 כשירות מותאמת ניתנת למחיקה אחרי ניקוי עובד שעזב',
    [ropeDeleted.duplicate, db._get(ST + '/schedule_qualifications/rope')], [false, null]);
