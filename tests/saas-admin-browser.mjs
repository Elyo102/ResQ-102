...
try {
  ...
  check('page head follows admin/maintenance conventions (rtl, viewport, theme.css, one initAppCheck, module script)',
    /<html lang="he" dir="rtl">/.test(html) && /name="viewport"/.test(html) && html.includes('./theme.css?v=42h28') && (html.match(/await initAppCheck\(app\);/g) || []).length === 1 && /<script type="module">/.test(html));
  check('every local import carries the release query', [...html.matchAll(/from '\.\/([^']+)'/g)].every((m) => /\?v=42h28$/.test(m[1])));
