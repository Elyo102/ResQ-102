...
const statusShadowManagerActive = { mode:'shadow', configured:true, manager:true,
  active:{ publication_id:'p_trial_live', revision:4, previous_publication_id:null, can_rollback:false } };
const statusShadowAfterPublish = { mode:'shadow', configured:true, manager:true,
  active:{ publication_id:'p_trial_new', revision:5, previous_publication_id:'p_trial_live', can_rollback:true } };
const statusShadowAfterRollback = { mode:'shadow', configured:true, manager:true,
  active:{ publication_id:'p_trial_rollback', revision:6, previous_publication_id:'p_trial_new', can_rollback:true } };
const setup = {
...
try {
  ...
  /* ------------------------------------------------------------------
   * Rollback gap acknowledgement: the first server response is a safe
   * refusal with the exact gap digest. The screen must show a second,
   * explicit confirmation and retry the same logical request with that
   * digest rather than inventing a new rollback.
   * ------------------------------------------------------------------ */
  const rollbackGapManager = await browser.newContext({ viewport:{ width:1440, height:1000 }, locale:'he-IL' });
  await prepare(rollbackGapManager, 'firefighter', {
    getScheduleRuntimeStatus:[{ data:statusAfterPublish }, { data:statusAfterRollback }],
    ...
    rollbackSchedule:[
      {
        ...
        message:'gap acknowledgement required',
        details:{
          schedule_code:'gaps-acknowledgement-required',
          detail:{
            digest:'rollback-gap-digest',
            summary:{ days:31, days_with_gaps:2, critical_gaps:0, other_gaps:2, station_minimum:7 },
            acknowledgeable:[
              { kind:'station', date:today, minimum:7, present:6, gap:1 },
              { kind:'qualification', date:tomorrow, key:'driver', label:'נהג', minimum:2, present:1, gap:1 }
            ],
            truncated:false
          }
        }
      },
      { data:{ publication_id:'p_rollback', revision:6, rolled_back_to:'p_live', notified_people:2 } }
    ]
  });
  const rollbackGapPage = await rollbackGapManager.newPage();
  const rollbackDialogs = [];
  rollbackGapPage.on('dialog', async (dialog) => {
    rollbackDialogs.push(dialog.message());
    await dialog.accept();
  });
  await rollbackGapPage.goto(base + '?tab=manage', { waitUntil:'load' });
  await rollbackGapPage.locator('#appMain:not(.hide)').waitFor();
  await test('rollback retries once with the exact noncritical-gap acknowledgement', async () => {
    await rollbackGapPage.locator('#rollback').click();
    await rollbackGapPage.locator('#rollbackMessage .ok').waitFor();
    assert.equal(rollbackDialogs.length, 2);
    assert.match(rollbackDialogs[1], /2.*פערי תקן או כשירות/);
    assert.match(rollbackDialogs[1], /סה״כ בתחנה.*6 מתוך 7/);
    assert.match(rollbackDialogs[1], /נהג.*1 מתוך 2/);
    const calls = await rollbackGapPage.evaluate(() => window.__CALLABLE_CALLS);
    const rollbacks = calls.filter((entry) => entry.name === 'rollbackSchedule');
    assert.equal(rollbacks.length, 2);
    assert.equal(rollbacks[0].payload.gap_acknowledgement, undefined);
    assert.equal(rollbacks[1].payload.gap_acknowledgement, 'rollback-gap-digest');
    assert.equal(rollbacks[1].payload.request_id, rollbacks[0].payload.request_id);
    assert.equal(rollbacks[1].payload.expected_active_publication_id, 'p_new');
    assert.equal(rollbacks[1].payload.target_publication_id, 'p_live');
  });
  await rollbackGapManager.close();
  ...
  await prepare(retryManager, 'firefighter', {
    ...
    publishSchedule:[
      ...
      { data:{ prepared:false, trial:true, publication_id:'p_retry_1', revision:1,
        notified_people:0, blocked_notifications:2 } },
      { data:{ prepared:false, trial:true, publication_id:'p_retry_2', revision:2,
  ...
  await test('a lost publish response retries the exact same request id', async () => {
    ...
    assert.match(await retryManagerPage.locator('#publishMessage').textContent(), /פורסם ופועל בסביבת הניסוי/);
  ...
  /* A planner response can be lost after the server has accepted the draft.
   * Until a terminal receipt arrives, the form is the identity of that pending
   * operation: month, horizon and every manual override must stay immutable. */
  const plannerRetryManager = await browser.newContext({
    viewport:{ width:1440, height:1000 }, locale:'he-IL'
  });
  const plannerRetryDraft = {
    draft_id:'draft_planner_retry', from:today, to:shiftDay(today, 60),
    summary:{ filled:90, blocking_gaps:0, days_below_minimum:0, rejected_manual:0 }
  };
  await prepare(plannerRetryManager, 'firefighter', {
    getScheduleRuntimeStatus:[{ data:statusShadowManager }],
    getScheduleManagerSetup:[{ data:setup }],
    runSchedulePlanner:[
      { reject:true, code:'functions/unavailable', message:'response lost' },
      { data:plannerRetryDraft }
    ],
    getScheduleDraftPreview:[{ data:Object.assign({}, draftPreview, {
      draft_id:'draft_planner_retry', expected_content_digest:'digest_planner_retry'
    }) }]
  });
  const plannerRetryPage = await plannerRetryManager.newPage();
  ...
  await test('lost planner response locks its full intent until the exact retry succeeds', async () => {
    ...
    await override.locator('[data-field="sub_station"]').selectOption('main');
    await override.locator('[data-field="person"]').selectOption('crew_1');
    await override.locator('[data-field="role"]').selectOption('driver');

    await plannerRetryPage.locator('#runPlanner').click();
    await plannerRetryPage.locator('#runMessage .err').waitFor();
    assert.equal(await plannerRetryPage.locator('#runPlanner').isEnabled(), true,
      'כשל עמום חסם את כפתור הניסיון החוזר');
    assert.equal(await plannerRetryPage.locator('#startMonth').isDisabled(), true);
    assert.equal(await plannerRetryPage.locator('#months').isDisabled(), true);
    assert.equal(await plannerRetryPage.locator('#addOverride').isDisabled(), true);
    assert.equal(await override.locator('input,select,button').evaluateAll((fields) =>
      fields.every((field) => field.disabled)), true,
    'אפשר לשנות חלק משיבוץ ידני בזמן שבקשת הטיוטה עדיין עמומה');

    await plannerRetryPage.locator('#runPlanner').click();
    await plannerRetryPage.locator('#previewMessage .ok').waitFor();
    ...
    assert.equal(await plannerRetryPage.locator('#addOverride').isEnabled(), true);
    assert.equal(await override.locator('input,select,button').evaluateAll((fields) =>
      fields.every((field) => !field.disabled)), true,
    'קבלה תקינה לא שחררה את פקדי כוונת הטיוטה');
  });
  await plannerRetryManager.close();
  ...
  await prepare(refreshFailureManager, 'firefighter', {
    ...
    publishSchedule:[{ data:{ prepared:false, trial:true, publication_id:'p_refresh_failure', revision:1,
      notified_people:0, blocked_notifications:2 } }]
  });
  ...
  await test('a refresh failure after success never sends publish twice', async () => {
    ...
    assert.match(await refreshFailurePage.locator('#publishMessage').textContent(), /פורסם ופועל בסביבת הניסוי/);
  ...
  await prepare(shadowManager, 'firefighter', {
    getScheduleRuntimeStatus:[
      { data:statusShadowManagerActive }, { data:statusShadowAfterPublish }, { data:statusShadowAfterRollback }
    ],
    getScheduleManagerSetup:[{ data:setup }],
    getMyScheduleV2:[{ data:legacyMine('shadow') }, { data:legacyMine('shadow') }],
    getStationScheduleRange:[{ data:legacyRange('shadow') }, { data:legacyRange('shadow') }, { data:legacyRange('shadow') }],
    ...
    publishSchedule:[{ data:{ prepared:false, trial:true, publication_id:'p_trial_new', revision:5,
      notified_people:0, blocked_notifications:2 } }],
    rollbackSchedule:[{ data:{ publication_id:'p_trial_rollback', revision:6,
      rolled_back_to:'p_trial_live', notified_people:0 } }]
  });
  ...
  await test('shadow manager publishes a reviewed trial schedule with delivery fenced to Eldad', async () => {
    ...
    assert.equal(await shadowManagerPage.locator('#publish').textContent(), 'פרסום לניסוי');
    ...
    const publish = publishCalls[0];
    assert.ok(publish, 'פרסום ניסוי ב-shadow לא קרא publishSchedule');
    ...
    assert.equal(await shadowManagerPage.locator('#rollback').isEnabled(), true,
      'פרסום ניסוי פעיל לא אפשר חזרה לגרסה הקודמת');

    await shadowManagerPage.locator('#rollback').click();
    await shadowManagerPage.locator('#rollbackMessage .ok').waitFor();
    const afterRollbackCalls = await shadowManagerPage.evaluate(() => window.__CALLABLE_CALLS || []);
    const rollback = afterRollbackCalls.find((entry) => entry.name === 'rollbackSchedule');
    assert.ok(rollback, 'rollback במצב ניסוי לא הגיע לשרת');
    assert.equal(rollback.payload.expected_active_publication_id, 'p_trial_new');
    assert.equal(rollback.payload.target_publication_id, 'p_trial_live');
    assert.equal(Object.hasOwn(rollback.payload, 'station_id'), false);
  ...
  * ================================================================== */
  ...
  /* פער של אדם שעזב חייב לכלול מסלול תיקון במסך. הוא אינו קיים עוד
   * ברשימת האנשים הפעילים, ולכן חיפוש רגיל לעולם לא יוכל לבחור אותו. */
  const invalidAssignmentReport = {
    target:{ kind:'active', publication_id:'p_live' }, station_minimum:0, gap_policy_revision:1,
    summary:{ days:1, days_with_gaps:1, critical_gaps:1, other_gaps:0, station_minimum:0 },
    blocking:[{ kind:'assignment', reason:'inactive_or_missing_person', date:today,
      key:'departed_1', uid:'departed_1', label:'עובד שעזב', sub_station:'main', minimum:1, present:0, gap:1 }],
    acknowledgeable:[], truncated:false, digest:null,
    days:[{ date:today, total:3, station_minimum:0, station_gap:0, station_candidates:[],
      sub_stations:[{ sub_station:'main', label:'אילת', people:3, minimum:2, gap:0, coverage:'ready' }],
      qualifications:[], invalid_assignments:[{ kind:'assignment', reason:'inactive_or_missing_person',
        date:today, key:'departed_1', uid:'departed_1', label:'עובד שעזב', sub_station:'main', minimum:1, present:0, gap:1 }],
      has_gap:true, has_critical_gap:true }]
  };
  const invalidCtx = await browser.newContext({ viewport:{ width:1200, height:1000 }, locale:'he-IL' });
  await prepare(invalidCtx, 'firefighter', {
    getScheduleRuntimeStatus:[{ data:statusEditable }],
    getScheduleManagerSetup:[{ data:setup }],
    getMyScheduleV2:[{ data:mine }],
    getStationScheduleRange:[{ data:stationRange }],
    getScheduleGapReport:[{ data:invalidAssignmentReport }],
    previewScheduleEdit:[{ data:editReport }]
  });
  const invalidPage = await invalidCtx.newPage();
  await invalidPage.goto(base + '?tab=manage', { waitUntil:'load' });
  await invalidPage.locator('#appMain:not(.hide)').waitFor();
  await test('active gap: a departed assignee can be queued for removal without appearing in active-person search', async () => {
    await invalidPage.locator('#gapLoad').click();
    const repair = invalidPage.getByRole('button', { name:'הוסף הסרה לעריכת הסידור' });
    await repair.waitFor();
    assert.match(await invalidPage.locator('#gapDays').textContent(), /עובד שעזב.*כבר אינו עובד פעיל/);
    await repair.click();
    assert.match(await invalidPage.locator('#editList').textContent(), /עובד שעזב.*הסרה מהסידור/);
    assert.equal(await invalidPage.locator('#editCheck').isEnabled(), true);
    await invalidPage.locator('#editCheck').click();
    const previewCall = (await invalidPage.evaluate(() => window.__CALLABLE_CALLS))
      .find((entry) => entry.name === 'previewScheduleEdit');
