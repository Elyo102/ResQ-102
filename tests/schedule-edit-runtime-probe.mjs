...
import { createFakeDb, seed, req, ST, SID, MGR, SHEET, buildRuntime, makeChecks, publishImportedSchedule, hash, stable, digest } from './_schedule-fake.mjs';
...
function activateChangedSource(db, previousSourceId) {
  const sourceId = 'src_changed';
  const previousBase = ST + '/schedule_sources/' + previousSourceId;
  const base = ST + '/schedule_sources/' + sourceId;
  const people = db._paths(previousBase + '/people/').map((path) =>
    Object.assign({ id: path.slice(path.lastIndexOf('/') + 1) }, db._get(path)))
    .filter((person) => person.id !== 'u2');
  people.push({
    id: 'u10', full_name: 'חבר חדש לבדיקה', active: true,
    sub_station: 'eilat', roles: ['ff']
  });
  people.sort((left, right) => String(left.id) < String(right.id) ? -1 : (String(left.id) > String(right.id) ? 1 : 0));
  people.forEach((person) => {
    const value = Object.assign({}, person);
    delete value.id;
    db._put(base + '/people/' + person.id, value);
  });
  const events = [{
    id: 'event_from_changed_source', title: 'אירוע מקור חדש',
    date: '2026-09-03', people: ['u10'], cancelled: false
  }];
  db._put(base + '/events/' + events[0].id, Object.assign({}, events[0], { id: undefined }));
  const basis = {
    station_id: SID, version: '2', revision: '2', carry: {},
    counts: { people: people.length, availability: 0, locked: 0, events: events.length },
    people, availability: {}, locked: {}, events
  };
  db._put(base, {
    station_id: SID, complete: true, version: '2', revision: '2',
    person_count: people.length, availability_count: 0, locked_count: 0,
    event_count: events.length, content_digest: digest(basis),
    content_key: hash(stable({ station_id: SID, people }))
  });
  db._put(ST + '/users/u10', {
    station_id: SID, station: SID, is_active: true, active: true,
    role: 'firefighter', full_name: 'חבר חדש לבדיקה'
  });
  const config = db._get(ST + '/schedule_state/runtime');
  db._put(ST + '/schedule_state/runtime', Object.assign({}, config, { active_source_id: sourceId }));
  return sourceId;
}

async function proveSnapshotBatchRecovery(failChunk, requestId, label) {
  const db = createFakeDb();
  const { rt } = await seed(db);
  const { pointer } = await publishImportedSchedule(db, rt);
  const edits = [{
    kind: 'absence', uid: 'u5', dates: ['2026-09-02'],
    absence: { kind: 'sick' }
  }];
  const report = await rt.previewScheduleEdit(req({ expected: expectedOf(pointer), edits }));
  const draftId = 'd_' + createHash('sha256')
    .update(SID + '|' + MGR + '|' + requestId, 'utf8').digest('hex').slice(0, 40);
  const draftPath = ST + '/schedule_drafts/' + draftId;
  let armed = true;
  let seenChunks = 0;
  let random = 0;
  const recovering = buildRuntime(db, {
    randomId: () => requestId + '-' + (++random),
    snapshotWriteChunkSize: 2,
    beforeSnapshotWriteChunk: async (event) => {
      if (!Array.isArray(event.paths)
          || !event.paths.some((path) => path.indexOf(draftPath + '/') === 0)) return;
      seenChunks += 1;
      if (armed && event.chunk_index === failChunk) {
        armed = false;
        throw new Error('simulated snapshot batch failure ' + failChunk);
      }
    }
  });
  const payload = {
    request_id: requestId, expected: expectedOf(pointer), edits,
    expected_edit_digest: report.edit_digest,
    gap_acknowledgement: report.gaps.digest
  };
  let failure = null;
  try { await recovering.applyScheduleEdit(req(payload)); } catch (error) { failure = error; }
  if (!failure || !/simulated snapshot batch failure/.test(String(failure.message || ''))) {
    throw failure || new Error('the injected snapshot failure did not run');
  }
  const staged = db._get(draftPath);
  const partialChildren = db._paths(draftPath + '/').filter((path) => path.split('/').length === 6);
  ok(label + '.1 הכשל המוזרק הגיע מ-batch היעד',
    failure && /simulated snapshot batch failure/.test(failure.message) && seenChunks === failChunk + 1,
    String(failure && failure.message) + ' chunks=' + seenChunks);
  eq(label + '.2 הטיוטה נשארה staging ולא הפכה לשימושית',
    [staged && staged.status, staged && staged.snapshot_complete === true], ['staging', false]);
  eq(label + '.3 מספר מסמכי-הבן החלקיים מדויק', partialChildren.length, failChunk * 2);

  const done = await recovering.applyScheduleEdit(req(payload));
  const complete = db._get(draftPath);
  eq(label + '.4 אותה בקשה השלימה את אותה טיוטה',
    [done.duplicate, done.revision, complete.status, complete.content_digest === complete.expected_snapshot_digest],
    [false, 2, 'complete', true]);
  const receipt = await recovering.applyScheduleEdit(req(payload));
  eq(label + '.5 replay מחזיר את קבלת ההודעות המקורית',
    [receipt.duplicate, receipt.publication_id, receipt.notified_people],
    [true, done.publication_id, done.notified_people]);
  ok(label + '.6 קבלת ההודעות אינה אפס', done.notified_people > 0, String(done.notified_people));
  eq(label + '.7 אין פרסום, outbox או יומן edit כפולים', [
    db._paths(ST + '/schedule_publications/').filter((path) => path.split('/').length === 4)
      .map((path) => db._get(path)).filter((value) => value && value.request_id === requestId).length,
    outboxOf(db, done.publication_id).length,
    auditOf(db).filter((value) => value.action === 'edit-draft' && value.request_id === requestId).length
...
/* 2 · דוח → ביצוע → revision חדש, יומן, outbox, לוח, rollback */
{
  const db = createFakeDb();
  const { rt } = await seed(db);
  const { pointer, published, imported, preview } = await publishImportedSchedule(db, rt);
  const directReplay = await rt.publish(req({
  ...
  eq('2.0א direct publish replay מחזיר את הקבלה המלאה', directReplay,
  ...
  eq('2.26 ניסיון חוזר → אותה קבלה מלאה, אותו פרסום, revision נשאר 2',
    [again.duplicate, again.publication_id, again.notified_people, db._get(ST + '/schedule_state/active').revision],
    [true, applied.publication_id, applied.notified_people, 2]);
  ok('2.26ב קבלת הניסיון החוזר אינה מאבדת את מונה ההודעות', applied.notified_people > 0, String(applied.notified_people));
...
/* 4 · 42H.2 ג׳ — שער הפערים בעריכה: פער קריטי חוסם; פער אחר דורש אישור חתום. */
{
  ...
  ok('4.5 getGapReport על הפרסום הפעיל: מועמד לראש משמרת ביום עם פער (u9 פנוי ב-3.9)', gapView.days.some((d) => d.has_critical_gap && d.qualifications.find((q) => q.key === 'shift_lead').candidates.some((c) => c.uid === 'u9')), JSON.stringify(gapView.days.map((d) => [d.date, d.qualifications.find((q) => q.key === 'shift_lead').candidates])));
  ...
  const policy2 = await rt.saveGapPolicy(req({ request_id: 'gp1b', station_minimum: 21, expected_revision: 1 }));
  const policy2Again = await rt.saveGapPolicy(req({ request_id: 'gp1b', station_minimum: 21, expected_revision: 1 }));
  eq('4.6ב ניסיון חוזר מחזיר את אותה קבלת revision',
    [policy2.revision, policy2Again.revision, policy2Again.duplicate], [2, 2, true]);
...
/* 5.4 §3 · העריכה מוצמדת למדיניות הפרסום; פרסום מיובא נערך מול התחנות הקנוניות בלבד. */
{
  ...
  // מיושרות למדיניות הפעילה (קו שחמון 0 → 2; קו אילת בפרסום מיובא קבוע על 7 —
  // חוזה המוצר של ההטלה הקנונית ב-main 2b12d98), הדוח אומר זאת, היומן רושם.
  ...
  eq('5.4.13 הלוח: קו שחמון עכשיו 2, אילת 7', [board3.days[0].sub_stations.find((s) => s.sub_station === 'shahmon').minimum, board3.days[0].sub_stations.find((s) => s.sub_station === 'eilat').minimum], [2, 7]);
...
/* 5.6 · כשל כתיבה במסמכי-הבן אינו הופך request_id תקין ליתום.
 * אותו payload משחזר את אותה תמונה בדיוק — גם לפני ה-batch הראשון וגם
 * אחרי batch חלקי — בלי פרסום, הודעה או audit כפולים. */
await proveSnapshotBatchRecovery(0, 'recover-first', '5.6 ראשון');
await proveSnapshotBatchRecovery(1, 'recover-middle', '5.7 אמצעי');

/* 6 · מקור כוח האדם השתנה: אין snapshot היברידי. אפשר להסיר אדם שעזב,
 * אבל אין להכניס לפרסום הערוך אדם או אירוע שהופיעו רק במקור החדש. */
{
  const db = createFakeDb();
  const { rt, sourceId } = await seed(db);
  const { pointer } = await publishImportedSchedule(db, rt, {
    request_id: 'source-drift-import', publish_id: 'source-drift-publish'
  });
  const basePublication = db._get(ST + '/schedule_publications/' + pointer.publication_id);
  const changedSourceId = activateChangedSource(db, sourceId);
  const edits = [{ kind: 'unassign', uid: 'u2', dates: ['2026-09-01', '2026-09-02'] }];
  const report = await rt.previewScheduleEdit(req({ expected: expectedOf(pointer), edits }));
  ok('6.1 הדוח חושף source drift ומתחייב לשמר את ה-snapshot',
    report.source_changed && report.source_changed.snapshot_preserved === true
      && report.source_changed.from.source_id === sourceId
      && report.source_changed.current.source_id === changedSourceId,
    JSON.stringify(report.source_changed));
  eq('6.2 הסרת מי שהושמט מהמקור החדש נשארת אפשרית', report.counts.changes, 1);
  eq('6.3 אחרי הסרת כל שיבוציו אין invalid assignment חוסם עבורו',
    report.gaps.blocking.some((gap) => gap.uid === 'u2'), false);
  await rejectsCode('6.4 אי אפשר לשבץ אדם שקיים רק במקור החדש', () =>
    rt.previewScheduleEdit(req({
      expected: expectedOf(pointer),
      edits: [{ kind: 'assign', uid: 'u10', dates: ['2026-09-03'], sub_station: 'timna', role: 'ff' }]
    })), 'edit-person-unknown');
  const done = await rt.applyScheduleEdit(req({
    request_id: 'source-drift-edit', expected: expectedOf(pointer), edits,
    expected_edit_digest: report.edit_digest,
    gap_acknowledgement: report.gaps.digest
  }));
  const publication = db._get(ST + '/schedule_publications/' + done.publication_id);
  eq('6.5 הפרסום הנגזר שומר את חתימת המקור של הבסיס ומפריד את מקור האימות', [
    publication.source_snapshot, publication.source_version, publication.source_revision,
    publication.source_digest, publication.snapshot_source_id, publication.source_id
  ], [
    basePublication.source_snapshot, basePublication.source_version, basePublication.source_revision,
    basePublication.source_digest, sourceId, changedSourceId
  ]);
  eq('6.6 אדם ואירוע מהמקור החדש לא נכנסו ל-snapshot הערוך', [
    db._get(ST + '/schedule_publications/' + done.publication_id + '/people/u10'),
    db._paths(ST + '/schedule_publications/' + done.publication_id + '/events/')
      .some((path) => (db._get(path) || {}).title === 'אירוע מקור חדש')
  ], [null, false]);
  ok('6.7 האדם שעזב נשאר ברשימת האנשים החתומה לצורכי היסטוריה',
    !!db._get(ST + '/schedule_publications/' + done.publication_id + '/people/u2'));
}
