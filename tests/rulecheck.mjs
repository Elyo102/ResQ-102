...
//   2. אילוץ       אפס קריאות get() — החלטה ארכיטקטונית
...
{
  // get() ו-exists() עולים קריאת מסמך לכל בדיקת הרשאה. כל
  // המערכת בנויה על claims כדי להימנע מזה.
  const gets = [...CODE.matchAll(/(?<![.\w])(get|exists|getAfter)\s*\(/g)];
  if (gets.length) fail(gets.length + ' קריאות get()/exists() — האילוץ הוא אפס',
    'כל אחת עולה קריאת מסמך בכל בדיקת הרשאה');
  else ok('אפס קריאות get() — כפי שתוכנן');
}
