...
async function probe(page, mode = 'pending') {
  return page.evaluate(async mode => {
    const sdk = await import('./monitored-functions.js?v=42h4');
...
try {
  for (const file of ['forms.html', 'attendance.html']) {
    await test(file + ': real App Check starts one monitor; finite global errors are deduplicated', async () => {
      ...
      try {
        ...
        // 42H.4 is in the finite client/server telemetry catalog, so global
        // errors retain the exact release instead of falling back to unknown.
        assert.deepEqual(calls[0].payload, { kind:'client-error', screen:file, version:'42H.4', code:'TypeError', callable:'unknown' });
        ...
        await f.page.evaluate(async () => {
          const { startMonitoring } = await import('./monitoring-bootstrap.js?v=42h4');
  ...
  await test('global and callable failures share the ten-attempt cap; failed reports cannot loop', async () => {
    ...
    try {
      ...
      await f.page.evaluate(async () => {
        const sdk = await import('./monitored-functions.js?v=42h4');
        const { TELEMETRY_CALLABLES } = await import('./incident-client.js?v=42h4');
  ...
  await test('all 21 Firebase screens bootstrap monitoring and all 14 factories use the facade', async () => {
    ...
    for (const file of consumers) {
      ...
      assert.ok(source.includes("from './monitored-functions.js?v=42h4'"), file);
