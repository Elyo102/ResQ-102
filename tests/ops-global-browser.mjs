...
async function probe(page, mode = 'pending') {
  return page.evaluate(async mode => {
    const sdk = await import('./monitored-functions.js?v=42h191');
...
try {
    ...
    await test(file + ': real App Check starts one monitor; finite global errors are deduplicated', async () => {
      ...
      try {
        ...
        await f.page.evaluate(async () => {
          const { startMonitoring } = await import('./monitoring-bootstrap.js?v=42h191');
  ...
  await test('global and callable failures share the ten-attempt cap; failed reports cannot loop', async () => {
    ...
    try {
      ...
      await f.page.evaluate(async () => {
        const sdk = await import('./monitored-functions.js?v=42h191');
        const { TELEMETRY_CALLABLES } = await import('./incident-client.js?v=42h191');
  ...
  await test('all 26 Firebase screens bootstrap monitoring and all 18 factories use the facade', async () => {
    ...
    for (const screen of screens) {
      ...
      if (module) {
        assert.ok(source.includes(`./${module}?v=42h191`), screen + ' disconnected bootstrap');
        source += fs.readFileSync(path.join(root, module), 'utf8');
      }
      assert.equal((source.match(/await initAppCheck\(app\);/g) || []).length, 1, screen);
    }
    const consumers = ['access.html','admin.html','alerts.html','attendance-shadow.html','attendance.html','callout.html','check.html','feedback.html','guards.html','import.html','login.html','schedule-management.js','stats.html','swaps.html','unlock.html','hr-client.js','hr-requests-client.js','hr-documents-client.js'];
    assert.equal(consumers.length, 18);
    for (const file of consumers) {
      const source = fs.readFileSync(path.join(root, file), 'utf8');
      assert.ok(source.includes("from './monitored-functions.js?v=42h191'"), file);
