...
async function probe(page, mode = 'pending') {
  return page.evaluate(async mode => {
    const sdk = await import('./monitored-functions.js?v=42h28');
...
try {
    ...
    await test(file + ': real App Check starts one monitor; finite global errors are deduplicated', async () => {
      ...
      try {
        ...
        await f.page.evaluate(async () => {
          const { startMonitoring } = await import('./monitoring-bootstrap.js?v=42h28');
  ...
  await test('global and callable failures share the ten-attempt cap; failed reports cannot loop', async () => {
    ...
    try {
      ...
      await f.page.evaluate(async () => {
        const sdk = await import('./monitored-functions.js?v=42h28');
        const { TELEMETRY_CALLABLES } = await import('./incident-client.js?v=42h28');
  ...
  await test('all 29 Firebase screens bootstrap monitoring and all 21 factories use the facade', async () => {
    ...
    for (const screen of screens) {
      ...
      if (module) {
        assert.ok(source.includes(`./${module}?v=42h28`), screen + ' disconnected bootstrap');
    ...
    for (const file of consumers) {
      ...
      assert.ok(source.includes("from './monitored-functions.js?v=42h28'"), file);
