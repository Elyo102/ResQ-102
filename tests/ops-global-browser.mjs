...
async function probe(page, mode = 'pending') {
  return page.evaluate(async mode => {
    const sdk = await import('./monitored-functions.js?v=42h15');
...
try {
    ...
    await test(file + ': real App Check starts one monitor; finite global errors are deduplicated', async () => {
      ...
      try {
        ...
        await f.page.evaluate(async () => {
          const { startMonitoring } = await import('./monitoring-bootstrap.js?v=42h15');
  ...
  await test('global and callable failures share the ten-attempt cap; failed reports cannot loop', async () => {
    ...
    try {
      ...
      await f.page.evaluate(async () => {
        const sdk = await import('./monitored-functions.js?v=42h15');
        const { TELEMETRY_CALLABLES } = await import('./incident-client.js?v=42h15');
  ...
  await test('all 25 Firebase screens bootstrap monitoring and all 17 factories use the facade', async () => {
    ...
    for (const screen of screens) {
      ...
      if (module) {
        assert.ok(source.includes(`./${module}?v=42h15`), screen + ' disconnected bootstrap');
    ...
    for (const file of consumers) {
      ...
      assert.ok(source.includes("from './monitored-functions.js?v=42h15'"), file);
