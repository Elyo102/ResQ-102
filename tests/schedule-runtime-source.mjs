...
/* ⭐ seq364/368 סעיף 4 · ניסיון חוזר של פרסום חייב להיות **אותה** בקשה.
 * מזהה אקראי בכל לחיצה הופך תשובה שאבדה לבקשה חדשה — `publication-conflict`
 * או כתיבה כפולה. המזהה נגזר מהטיוטה, מחתימת התוכן ומהכוונה. */
check('the publish request id is derived from the draft, not rolled per click', () => {
  const ui = stripComments(read('schedule-management.js'));
  const at = ui.indexOf('async function publishDraft(');
  const body = ui.slice(at, ui.indexOf('\nfunction setRollbackAvailability', at));
  assert.ok(body.indexOf('call.publish(') > -1, 'publishDraft אינו קורא ל-publish');
  assert.ok(!/request_id:\s*requestId\('publish'\)/.test(body),
    'מזהה הפרסום עדיין אקראי — ניסיון חוזר הוא בקשה חדשה');
  assert.ok(/request_id:\s*await stableRequestId\('publish',\s*\[\s*state\.draft\.draft_id,\s*state\.draftPreview\.expected_content_digest,\s*preparing \? 'prepare' : 'publish'\s*\]\)/.test(body),
    'המזהה אינו נגזר מטיוטה + חתימה + כוונה');
  // והגזירה עצמה: hash של החלקים, בתוך חוזה המזהה של השרת.
  const fnAt = ui.indexOf('async function stableRequestId(');
  const fn = ui.slice(fnAt, ui.indexOf('\n}\n', fnAt));
  assert.ok(fn.indexOf("subtle.digest('SHA-256'") > -1, 'הגזירה אינה hash');
  assert.ok(/\.slice\(0, 120\)/.test(fn), 'הנפילה-לאחור חורגת מחוזה המזהה (120 תווים)');
  assert.ok(!/randomUUID|Math\.random|Date\.now/.test(fn), 'הגזירה היציבה מכילה אקראיות');
});

assert.equal(passed, 109);
console.log('\n109 schedule runtime source checks passed.');
