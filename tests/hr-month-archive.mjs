...
const results=await page.evaluate(async()=>{
  ...
  await check('local hours files omit medical classification and free text',async()=>{let captured=[];const sensitive='MEDICAL-SENTINEL-PRIVATE';const row={date:'2026-09-01',hours:0,day_type_he:'מחלה',start:'',end:'',start2:'',end2:'',site_name:'מרפאה',notes:sensitive,overtime_reason:sensitive,reason:sensitive,end_day:null,end_day2:null};const a={...adapter,getEmployeeMonth:async()=>({report:{...make(),rows:[row]},freshness:{source:'server'}})};for await(const file of buildLocalMonthFiles(a,'2026-09',{pdf:async(_title,lines)=>{captured=lines;return new TextEncoder().encode('%PDF-redacted');}}))void file;const text=captured.join('\n');ok(text.includes('2026-09-01')&&text.includes('שעות: 0'),'hours ledger retained');ok(!text.includes('מחלה')&&!text.includes('מרפאה')&&!text.includes(sensitive),'medical and free text redacted');});
...
await page.evaluate(async()=>{const {createMonthArchiveUI}=await import('/hr-month-archive-ui.js');document.body.innerHTML='<div id="host"><div class="hr-toolbar"><input data-hr="month" value="2026-09"></div></div>';window.live=true;window.listeners=[];window.downloadClicks=0;const click=HTMLAnchorElement.prototype.click;HTMLAnchorElement.prototype.click=function(){window.downloadClicks++;window.lastDownloadName=this.download;return click.call(this);};window.hostAdapter={...window.archiveFixture,currentSession:()=>window.live?{uid:'hr',stationId:'test',role:'hr_coordinator',epoch:1}:null,subscribeIdentity:f=>{window.listeners.push(f);return()=>{};}};window.hostUI=createMonthArchiveUI(document.getElementById('host'),window.hostAdapter);});
const downloadEvent=page.waitForEvent('download');await page.getByRole('button',{name:'הורדת כל דוחות החודש'}).click();const download=await downloadEvent;await download.saveAs(path.join(output,'actual-archive.zip'));const anchorDownloadName=await page.evaluate(()=>window.lastDownloadName);assert.match(anchorDownloadName,/ספטמבר 2026/);passed++;console.log('PASS actual host download with real PDF generation (real Hebrew filename read from the anchor element itself — see comment on Playwright/Chromium suggestedFilename() limitation for non-ASCII names)');
