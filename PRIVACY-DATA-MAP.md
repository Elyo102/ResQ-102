...
| 9 | **תפעולי — נוכחות ושעות** | `emp_number`, `crew`, שעות, סטטוס | `stations/{sid}/attendance/{docId}` · `stations/{sid}/monthly_reports/{docId}` · `stations/{sid}/attendance_correction_receipts/{receiptId}` | קריאה לפי תפקיד/בעלות. כתיבה רגילה, חישוב והגשת חודש נעשים ב-callable עם זהות שרת, App Check ו-CAS; כללי הלקוח מאפשרים יצירה ישירה רק למסלול חופשה מאושרת ולייבוא היסטורי מוגבל, וחוסמים עדכון/מחיקה ישירים. ביטול טיוטת עובד מסיר אותה מהחודש הפעיל אך שומר before-image מוגבל בקבלה שרתית | `retain_indefinitely_no_automatic_deletion` לרשומות המקור ולדוחות; דיווחים שהוגשו/אושרו אינם נמחקים. ביטול טיוטה נשמר כראיית שרת בקבלה. יכולת שחזור מגיבוי עדיין מוגבלת לחלון הגיבוי המוגדר | `restricted_identity` |
