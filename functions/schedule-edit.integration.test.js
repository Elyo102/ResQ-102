...
(async function run() {
  ...
  await test('seed: import the sheet, switch to new (test-only direct write), publish revision 1', async () => {
    ...
    const published = await api.publish(req(MGR, { request_id: 'edit_seed_publish', draft_id: imported.draft_id, expected_content_digest: preview.expected_content_digest }));
