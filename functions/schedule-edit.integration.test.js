...
function runtime(hooks) {
  return createScheduleRuntime({
    ...
    beforeSnapshotWriteChunk: hooks && hooks.beforeSnapshotWriteChunk,
...
async function snapshotChildCount(ref) {
  const collections = await ref.listCollections();
  let count = 0;
  for (const collection of collections) count += (await collection.get()).size;
  return count;
}
...
const SHEET = [
  ...
  // Keep this edit-suite baseline valid. Conflicting/duplicate absences are
  // covered (and blocked) by the dedicated sheet-import suite.
  row(['בצפון', '', '', '']),
  row(['באילת', '', '', 'רועי כהן'])
].join('\n');
...
async function activateChangedSource(previousSourceId) {
  const sourceId = 'sheet_src_changed';
  const sourceRef = station().collection('schedule_sources').doc(sourceId);
  const previousRef = station().collection('schedule_sources').doc(previousSourceId);
  const previousPeople = await previousRef.collection('people').get();
  const people = previousPeople.docs
    .filter((doc) => doc.id !== 'u2')
    .map((doc) => Object.assign({ id: doc.id }, doc.data() || {}));
  people.push({
    id: 'u10', full_name: 'חבר חדש לבדיקה', active: true,
    sub_station: 'eilat', roles: ['ff']
  });
  people.sort((left, right) => left.id < right.id ? -1 : (left.id > right.id ? 1 : 0));
  const events = [{
  ...
  people.forEach((person) => {
    const value = Object.assign({}, person); delete value.id;
    batch.set(sourceRef.collection('people').doc(person.id), value);
  });
  events.forEach((event) => {
    const value = Object.assign({}, event); delete value.id;
    batch.set(sourceRef.collection('events').doc(event.id), value);
  });
  const basis = {
  ...
  batch.set(station().collection('users').doc('u10'), {
    station: SID, role: 'firefighter', full_name: 'חבר חדש לבדיקה', active: true
  });
  batch.set(runtimeDoc(), { active_source_id: sourceId }, { merge: true });
  await batch.commit();
  return sourceId;
}
...
(async function run() {
  ...
  let gapDigest = null;
  const edits = [
    { kind: 'assign', uid: 'u1', dates: ['2026-09-01'], sub_station: 'shahmon' },
    { kind: 'assign', uid: 'u1', dates: ['2026-09-02'], sub_station: 'timna' },
  ...
  await test('the board shows the edit; a retry returns the same receipt; the old base is refused', async () => {
    ...
    assert.deepEqual(
      [again.duplicate, again.publication_id, again.notified_people],
      [true, applied.publication_id, applied.notified_people]
    );
    assert.ok(applied.notified_people > 0, 'the replay receipt must preserve the original notification count');
  ...
  await test('§3/§6 edits are refused for a foreign sub-station key and an unknown role; after a policy change the rows are rebased, not refused', async () => {
    ...
    const rebased = await api.previewScheduleEdit(req(MGR, { expected: expectedOf(current), edits: [{ kind: 'unassign', uid: 'u7', dates: ['2026-09-02'] }] }));
    ...
    const unacked = await caught(() => api.applyScheduleEdit(req(MGR, { request_id: 'edit_rebase', expected: expectedOf(current), edits: [{ kind: 'unassign', uid: 'u7', dates: ['2026-09-02'] }], expected_edit_digest: rebased.edit_digest, gap_acknowledgement: rebased.gaps.digest })));
  ...
  async function proveSnapshotRecovery(failChunk, requestId, absence) {
    const current = (await station().collection('schedule_state').doc('active').get()).data();
    const mine = [{ kind: 'absence', uid: 'u5', dates: ['2026-09-02'], absence }];
    const report = await api.previewScheduleEdit(req(MGR, { expected: expectedOf(current), edits: mine }));
    const draftId = 'd_' + hash(SID + '|' + MGR + '|' + requestId).slice(0, 40);
    ...
    const recovering = runtime({
      ...
      beforeSnapshotWriteChunk: async (event) => {
        if (!Array.isArray(event.paths)
            || !event.paths.some((path) => path.startsWith(draftRef.path + '/'))) return;
        chunks += 1;
    ...
    const payload = {
      request_id: requestId, expected: expectedOf(current), edits: mine,
      expected_edit_digest: report.edit_digest,
      gap_acknowledgement: report.gaps.digest
    };
    const failure = await caught(() => recovering.applyScheduleEdit(req(MGR, payload)));
    assert.match(String(failure && failure.message), /simulated snapshot batch failure/);
    assert.equal(chunks, failChunk + 1);
    const staged = (await draftRef.get()).data() || {};
    assert.equal(staged.status, 'staging');
    assert.notEqual(staged.snapshot_complete, true);
    assert.equal(await snapshotChildCount(draftRef), failChunk * 2);

    const done = await recovering.applyScheduleEdit(req(MGR, payload));
    const completed = (await draftRef.get()).data() || {};
    assert.deepEqual(
      [done.duplicate, done.revision, completed.status,
        completed.content_digest === completed.expected_snapshot_digest],
      [false, Number(current.revision) + 1, 'complete', true]
    );
    const again = await recovering.applyScheduleEdit(req(MGR, payload));
    assert.deepEqual(
      [again.duplicate, again.publication_id, again.notified_people],
      [true, done.publication_id, done.notified_people]
    );
    assert.ok(done.notified_people > 0);
    const publications = (await station().collection('schedule_publications').get()).docs
      .map((doc) => doc.data()).filter((value) => value.request_id === requestId);
    const outbox = await station().collection('schedule_publications').doc(done.publication_id)
      .collection('schedule_outbox').get();
    const audits = (await station().collection('schedule_audit').get()).docs
      .map((doc) => doc.data()).filter((value) => value.action === 'edit-draft' && value.request_id === requestId);
    assert.deepEqual([publications.length, outbox.size, audits.length], [1, done.notified_people, 1]);
  }

  await test('snapshot recovery: exact retry resumes a failure before the first child batch', async () => {
    // u5 is absent from the preceding test; remove that absence.
    await proveSnapshotRecovery(0, 'edit_recover_first', null);
  });
  ...
  await test('rollback returns only to the immediate previous publication and obeys the gap gate', async () => {
    ...
    assert.equal(back.days[0].absences.some((a) => a.uid === 'u5'), false);
  });

  await test('source drift preserves the signed snapshot while allowing removal of a departed assignee', async () => {
    const current = (await station().collection('schedule_state').doc('active').get()).data();
    const baseRef = station().collection('schedule_publications').doc(current.publication_id);
    const base = (await baseRef.get()).data() || {};
    const config = (await runtimeDoc().get()).data() || {};
    const changedSourceId = await activateChangedSource(config.active_source_id);
    const edits = [{ kind: 'unassign', uid: 'u2', dates: ['2026-09-01', '2026-09-02'] }];
    const report = await api.previewScheduleEdit(req(MGR, { expected: expectedOf(current), edits }));
    assert.equal(report.source_changed && report.source_changed.snapshot_preserved, true);
    assert.equal(report.source_changed.from.source_id, config.active_source_id);
    assert.equal(report.source_changed.current.source_id, changedSourceId);
    assert.equal(report.counts.changes, 2, 'departed assignee removal was blocked');
    assert.equal(report.gaps.blocking.some((gap) => gap.uid === 'u2'), false);
    const refused = await caught(() => api.previewScheduleEdit(req(MGR, {
      expected: expectedOf(current),
      edits: [{ kind: 'assign', uid: 'u10', dates: ['2026-09-03'], sub_station: 'timna', role: 'ff' }]
    })));
    assert.equal(refused && refused.code, 'edit-person-unknown');
    const done = await api.applyScheduleEdit(req(MGR, {
      request_id: 'edit_source_drift', expected: expectedOf(current), edits,
      expected_edit_digest: report.edit_digest, gap_acknowledgement: report.gaps.digest
    }));
    const publicationRef = station().collection('schedule_publications').doc(done.publication_id);
    ...
    assert.deepEqual([
      ...
      base.source_snapshot, base.source_version, base.source_revision,
      base.source_digest, config.active_source_id, changedSourceId
    ]);
    assert.equal((await publicationRef.collection('people').doc('u10').get()).exists, false);
    assert.equal((await publicationRef.collection('people').doc('u2').get()).exists, true);
    const storedEvents = await publicationRef.collection('events').get();
    assert.equal(storedEvents.docs.some((doc) => (doc.data() || {}).title === 'אירוע מקור חדש'), false);
  });
