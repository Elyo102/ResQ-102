...
(async function run() {
  ...
  await test('create, retry, conflict, CAS update and audit on a real transaction', async () => {
    ...
    assert.deepEqual([again.duplicate, again.key, again.revision], [true, 'diver', 1]);
  ...
  await test('clearing keeps a monotonic tombstone, rejects stale rev0, and safely releases a departed member holding', async () => {
    const cleared = await api.setPersonQualifications(req(MGR, {
      request_id: 'h_clear_u2', person: 'u2', qualifications: [], expected_revision: 2
    }));
    assert.deepEqual([cleared.duplicate, cleared.qualifications, cleared.revision], [false, [], 3]);
    const tombstone = (await station().collection('schedule_person_qualifications').doc('u2').get()).data();
    assert.deepEqual([tombstone.qualifications, tombstone.revision, tombstone.cleared], [[], 3, true]);
    await api.setPersonQualifications(req(MGR, {
      request_id: 'h_after_clear_u2', person: 'u2', qualifications: ['driver'], expected_revision: 3
    }));
    const replay = await api.setPersonQualifications(req(MGR, {
      request_id: 'h_clear_u2', person: 'u2', qualifications: [], expected_revision: 2
    }));
    assert.deepEqual([replay.duplicate, replay.qualifications, replay.revision], [true, [], 3]);
    const current = (await station().collection('schedule_person_qualifications').doc('u2').get()).data();
    assert.deepEqual([current.qualifications, current.revision, current.cleared], [['driver'], 4, false]);
    const stale = await caught(() => api.setPersonQualifications(req(MGR, {
      request_id: 'h_aba_u2', person: 'u2', qualifications: ['driver'], expected_revision: 0
    })));
    assert.equal(stale && stale.code, 'holdings-revision-stale');

    await station().collection('users').doc('u3').update({ active: false });
    const departedAssign = await caught(() => api.setPersonQualifications(req(MGR, {
      request_id: 'h_departed_assign', person: 'u3', qualifications: ['driver'], expected_revision: 1
    })));
    assert.equal(departedAssign && departedAssign.code, 'person-not-member');
    const departedClear = await api.setPersonQualifications(req(MGR, {
      request_id: 'h_departed_clear', person: 'u3', qualifications: [], expected_revision: 1
    }));
    assert.deepEqual([departedClear.qualifications, departedClear.revision], [[], 2]);
    const departedReplay = await api.setPersonQualifications(req(MGR, {
      request_id: 'h_departed_clear', person: 'u3', qualifications: [], expected_revision: 1
    }));
    assert.deepEqual([departedReplay.duplicate, departedReplay.qualifications, departedReplay.revision], [true, [], 2]);
    const noHolding = await caught(() => api.setPersonQualifications(req(MGR, {
      request_id: 'h_departed_missing', person: 'departed_without_holdings', qualifications: []
    })));
    assert.equal(noHolding && noHolding.code, 'person-not-member');

    const deleted = await api.deleteQualification(req(MGR, {
      request_id: 'd_pilot_after_departure', key: 'pilot', expected_revision: 1
    }));
    assert.equal(deleted.duplicate, false);
    assert.equal((await station().collection('schedule_qualifications').doc('pilot').get()).exists, false);
  });
