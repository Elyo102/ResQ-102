 ...
 *
 * ⭐ `station_commander` **אינו** ברשימה במכוון.
 *
 * ההכרעה שהתקבלה נקבה בשמות `commander` ו-`deputy`. יש בסכימה גם
 * `station_commander`, ולא ברור אם הכוונה הייתה לכלול אותו. לכלול
 * אותו „כי זה הגיוני" פירושו להרחיב הרשאה על סמך ניחוש שלי, וזה
 * הכיוון הלא נכון לטעות בו. הרשימה נשארת צרה עד להכרעה מפורשת.
 */
