...
const DATA_POLICIES = Object.freeze([
  ...
  // 42H.21 · קליטה בקישור קבוצתי. הקמפיין מכיל hash של הטוקן (לא סוד
  // גולמי) ומונים; הנרשמים — הצהרות כשירות והערות ביקורת על אנשים.
  policy('join_campaigns/{campaignId}', 'root', 'source_of_truth',
    'count_drop', 'managed_export', 'restore', 'confidential',
    'policy_required_before_wiring',
    'Group join campaigns: token hash, quota and status (no raw secret).',
    { humanReadable:'redacted' }),
  policy('join_campaigns/{campaignId}/registrants/{uid}', 'root', 'source_of_truth',
    'count_drop', 'managed_export', 'restore_after_parent', 'restricted_identity',
    'policy_required_before_wiring',
    'Per-registrant join record: shift choice, qualification declarations, review notes.',
    { humanReadable:'forbidden' }),
  policy('join_registrant_index/{uid}', 'root', 'derived', 'count_drop', 'rebuild',
    'rebuild', 'restricted_identity', 'rebuild_not_retain',
    'Pointer from a registrant UID to its campaign; rebuildable from registrants.',
    { humanReadable:'forbidden' }),
  policy('join_campaign_inspect_quota/{quotaId}', 'root', 'temporary', 'none', 'exclude',
    'do_not_restore', 'operational', 'ttl_policy_required',
    'Hourly rate-limit counters for anonymous join-link inspection.', { humanReadable:'redacted' }),  // שכבת SaaS מסחרית. ארגון = הפניות למזהי תחנות בלבד (לא נתוני תחנה);
  ...
  policy('stations/{sid}/sub_stations/{subId}/bulletin_messages/{messageId}/bulletin_replies/{replyId}',
    ...
    { humanReadable:'redacted' }),
  policy('stations/{sid}/bulletin_view_receipts/{messageId}/bulletin_view_recipients/{uid}',
    'station', 'temporary', 'activity', 'exclude', 'do_not_restore',
    'restricted_identity', 'ttl_400_days',
    'Private display evidence expires automatically and must not be extended by backup restore.',
    { humanReadable:'forbidden' }),
  ...
  policy('stations/{sid}/submissions/{subId}', 'station', 'source_of_truth',
    ...
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/form_submission_operations/{operationId}', 'station', 'source_of_truth',
    'integrity_group', 'managed_export', 'restore_after_parent', 'restricted_identity',
    'same_retention_as_submission',
    'Private idempotency receipt must be restored with its canonical submission.',
    { consistencyGroup:'form_submission', humanReadable:'forbidden' }),
  ...
  policy('stations/{sid}/schedule_state/{stateId}', 'station',
    ...
    { humanReadable:'redacted' }),
  policy('stations/{sid}/schedule_publication_months/{month}', 'station',
    'source_of_truth', 'count_any_loss', 'managed_export', 'restore',
    'restricted_identity', 'legal_retention_policy_required',
    'Current signed publication owner or unowned tombstone for one calendar month.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/schedule_publication_authority_operations/{operationId}', 'station',
    'source_of_truth', 'count_any_loss', 'managed_export', 'restore',
    'restricted_identity', 'legal_retention_policy_required',
    'Immutable monthly publication authority receipt required for deterministic rollback.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/schedule_access/{uid}', 'station',
    ...
    { consistencyGroup:IDENTITY_CONSISTENCY_GROUP, humanReadable:'forbidden' }),
  policy('stations/{sid}/schedule_qualifications/{qualificationId}', 'station',
    'source_of_truth', 'count_any_loss', 'managed_export', 'restore',
    'confidential', 'policy_required_before_wiring',
    'Schedule qualification catalog and minimum staffing requirements.',
    { humanReadable:'redacted' }),
  policy('stations/{sid}/schedule_person_qualifications/{uid}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Versioned member qualification holdings, including cleared-state tombstones.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/schedule_qualification_audit/{entryId}', 'station',
    'audit_log', 'activity', 'managed_export', 'restore',
    'restricted_identity', 'audit_retention_policy_required',
    'Who changed the schedule qualification catalog or member holdings, and when.',
    { humanReadable:'forbidden' }),
  ...
  policy('stations/{sid}/device_readiness/{uid}', 'station', 'temporary',
    'none', 'exclude', 'do_not_restore', 'restricted_identity', 'ttl_policy_required',
    'Device readiness challenge state (token hash, nonce hash, attempts); recomputed on demand.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/live_lab_config/{docId}', 'station', 'temporary',
    'none', 'exclude', 'do_not_restore', 'restricted_identity', 'ttl_1_day',
    'Personal live-lab activation is temporary control state.', { humanReadable:'forbidden' }),
  policy('stations/{sid}/live_lab_probes/{probeId}', 'station', 'temporary',
    'none', 'exclude', 'do_not_restore', 'restricted_identity', 'ttl_policy_required',
    'Bounded push-probe receipts are not business records.', { humanReadable:'forbidden' }),
  policy('stations/{sid}/live_lab_quotas/{quotaId}', 'station', 'temporary',
    'none', 'exclude', 'do_not_restore', 'restricted_identity', 'ttl_policy_required',
    'Per-user live-lab rate-limit state.', { humanReadable:'forbidden' }),
  ...
  policy('stations/{sid}/callouts/{coId}', 'station', 'source_of_truth',
    'count_drop', 'managed_export', 'restore', 'restricted_identity',
    'legal_retention_policy_required', 'Callout records.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/callouts/{coId}/responses/{responseUid}', 'station', 'source_of_truth',
    'count_drop', 'managed_export', 'restore', 'restricted_identity',
    'legal_retention_policy_required', 'Private callout response and rejection reason.',
    { humanReadable:'forbidden' }),
  ...
  // business/operation-receipt restore procedure is still unresolved; the
  // restore labels below are dependencies, not a claim that restore is safe.
  // Delivery jobs, attempts and unknown outcomes must never be restored into
  // active queues. Existing records are not deleted by this policy.
  policy('stations/{sid}/hr_requests/{requestId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private request state; prospective classification only, no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_requests/{requestId}/events/{eventId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Immutable private messages and case events require a coherent parent snapshot; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_request_operations/{operationId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
  ...
  policy('stations/{sid}/hr_documents/{documentId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private targeted documents and procedures; prospective classification only, no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_documents/{documentId}/revisions/{revision}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Immutable private content revisions require a coherent parent snapshot; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_documents/{documentId}/revisions/{revision}/receipts/{uid}', 'station',
    'audit_log', 'activity', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Opened and acknowledgment evidence belongs to the exact immutable revision, not proof of reading; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_document_operations/{operationId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Idempotency receipts require a coherent same-snapshot document/revision restore; that procedure remains unresolved. No export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_nudge_actions/{actionId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Manual nudge generation and replay state must not restart after restore; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_nudge_intents/{intentId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private delivery attempts and unknown outcomes must not be restored or replayed; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_request_notification_jobs/{jobId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private request notification generation must not be restored into an active queue; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_document_notification_jobs/{jobId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private document notification generation must not be restored into an active queue; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_domain_notification_intents/{intentId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private domain delivery attempts and unknown outcomes must not be restored or replayed; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('hr_nudge_bulk_locks/{lockId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Global actor/month locks are live ownership, not restorable business data; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('hr_nudge_actor_quotas/{quotaId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Global nudge actor rate limits must not be restored; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('hr_request_actor_quotas/{quotaId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Global request actor rate limits must not be restored; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_hours_review_events/{reviewId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'specialized_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Durable private inspection and idempotency evidence. Preserve original reviewed_revision; restored Firestore updateTime cannot manufacture current inspection. Coherent restore remains unresolved; no export, restore or deletion activated; never replay notifications.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_hours_reviews/{summaryId}', 'station',
    'derived', 'required_document_shape', 'managed_export', 'specialized_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private latest-receipt projection. Preserve original reviewed_revision; restored Firestore updateTime cannot manufacture current inspection. Coherent restore remains unresolved, no rebuild implemented; no export, restore or deletion activated; never replay notifications.',
    { humanReadable:'forbidden' }),
  policy('hr_hours_review_actor_quotas/{quotaId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Global inspection actor rate limits must not be restored; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_hours_review_notification_jobs/{jobId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private inspection notification work must never restore into an active queue; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/attendance_correction_events/{correctionId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'specialized_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Immutable correction before/after evidence survives row deletion. Coherent same-snapshot event/receipt/attendance restore remains unresolved; preserve original versions and attribution; no export, restore or deletion activated; never replay notifications.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/attendance_correction_receipts/{correctionId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'specialized_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Durable correction idempotency is independent of transient notification jobs. Coherent same-snapshot event/receipt/attendance restore remains unresolved; preserve original versions and attribution; no export, restore or deletion activated; never replay notifications.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/attendance_correction_notification_jobs/{jobId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private correction notification work must never restore into an active queue; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_workforce_cases/{recordId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'specialized_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private HR follow-up state; restore only with its immutable events and operation receipts; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_workforce_cases/{recordId}/events/{eventId}', 'station',
    'audit_log', 'activity', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Immutable HR before/after evidence contains sensitive reasons and actor identity; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_workforce_operations/{operationId}', 'station',
    'source_of_truth', 'count_drop', 'managed_export', 'restore_after_parent',
    'restricted_identity', 'policy_required_before_wiring',
    'Idempotency receipts must remain coherent with the HR case mutation; no export, restore or deletion activated.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_workforce_reminder_locks/{lockId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Reminder ownership must never be replayed after restore; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('stations/{sid}/hr_workforce_notification_jobs/{jobId}', 'station',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Private reminder work must never restore into an active queue; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('hr_workforce_actor_quotas/{quotaId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
    'Global HR workforce actor quota is live rate-limit state; exclusion activates no deletion.',
    { humanReadable:'forbidden' }),
  policy('hr_document_actor_quotas/{quotaId}', 'root',
    'temporary', 'none', 'exclude', 'do_not_restore',
    'restricted_identity', 'policy_required_before_wiring',
