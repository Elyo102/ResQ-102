...
function normalizeEdits(raw, range) {
  ...
  const from = plain(range) ? String(range.from || '') : '';
  const to = plain(range) ? String(range.to || '') : '';
  if (!plain(range) || !validIsoDate(from) || !validIsoDate(to) || from > to) {
  ...
  return raw.map((item, i) => {
    ...
    const out = { kind: item.kind, uid, dates: normalizeDates(item.dates, from, to, index) };
    if (item.kind === 'assign') {
