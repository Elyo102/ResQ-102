...
const base=[['','1/9','2/9','3/9'],['תחנה א','עובד בדיקה 1','',''],['תחנה ב','','עובד בדיקה 2','']];
...
test('duplicate assignment across stations on same day is a warning, not a blocker',()=>{const grid=[['','1/9','2/9','3/9'],['א','עובד בדיקה 1','',''],['ב','עובד בדיקה 1','','']];const out=pipeline.buildWorkbookImport({station_id:'eilat_102',month:'2026-09',input:grid,inventory:[],bindings:[]});assert.equal(out.blockers.length,0);assert.ok(out.warnings.some(w=>w.code==='duplicate-assignment'&&w.count===1));});
test('duplicate station label is rejected as ambiguous',()=>{const grid=[['','1/9','2/9','3/9'],['א','עובד בדיקה 1','',''],['א','','עובד בדיקה 2','']];assert.throws(()=>pipeline.buildWorkbookImport({station_id:'eilat_102',month:'2026-09',input:grid,inventory:[],bindings:[]}),e=>e.code==='ambiguous-station-label');});
