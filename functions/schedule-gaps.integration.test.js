...
const SHEET = [
  ...
  // Keep the gap-suite baseline free of an assignment/absence conflict.
  row(['בצפון', '', '', '']),
  row(['באילת', '', '', 'רועי כהן'])
].join('\n');
...
(async function run() {
  ...
  await test('the canonical live roster, including the in-transaction fence, excludes a departed source member', async () => {
    const userRef = station().collection('users').doc('u1');
    await userRef.set({ active: false }, { merge: true });
    const afterDeparture = await api.getGapReport(req(MGR, { draft_id: imported.draft_id }));
    assert.ok(afterDeparture.blocking.some((gap) =>
      gap.kind === 'assignment' && gap.reason === 'inactive_or_missing_person' && gap.uid === 'u1'),
    JSON.stringify(afterDeparture.blocking));
    await userRef.set({ active: true }, { merge: true });

    const checked = await api.previewScheduleImport(req(MGR, { month: '2026-09', paste: SHEET, aliases }));
    const next = await api.importScheduleSheet(req(MGR, {
      request_id: 'gap_live_roster_import', month: '2026-09', paste: SHEET, aliases,
      expected_report_digest: checked.report_digest
    }));
    const nextPreview = await api.getDraftPreview(req(MGR, { draft_id: next.draft_id, start: '2026-09-01' }));
    let changed = false;
    const fenced = runtime({
      beforeSnapshotFinalize: async (info) => {
        if (!changed && info && info.kind === 'publication') {
          changed = true;
          await userRef.set({ active: false }, { merge: true });
        }
      }
  ...
  await test('a critical qualification minimum with no holder blocks a new draft publish; a candidate is offered, never assigned', async () => {
    ...
    // u6 מחזיק ראש משמרת; ביום שבו הוא פנוי ולא נעדר הוא מועמד — והטיוטה לא השתנתה.
    const dayWithCandidate = report.days.find((d) => d.qualifications.find((q) => q.key === 'shift_lead').candidates.some((c) => c.uid === 'u6'));
    assert.ok(dayWithCandidate, 'u6 offered as a candidate somewhere');
