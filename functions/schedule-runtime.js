...
function createScheduleRuntime(deps) {
  ...
  const beforeSnapshotWriteChunk = typeof d.beforeSnapshotWriteChunk === 'function'
  ...
  // Test-only seam: production always keeps the audited 350-write ceiling.
  // A small value lets the recovery probes fail a real first/middle snapshot
  // batch without manufacturing hundreds of schedule rows.
  const snapshotWriteChunkSize = integer(d.snapshotWriteChunkSize)
      && d.snapshotWriteChunkSize >= 1 && d.snapshotWriteChunkSize <= MAX_BATCH_WRITES
    ? d.snapshotWriteChunkSize : MAX_BATCH_WRITES;
  ...
  function liveLabRef(sid) {
    return stationRef(sid).collection('live_lab_config').doc('current');
  }
  ...
  async function commitWrites(ops, options) {
    const opts = plain(options) ? options : {};
    const chunkSize = opts.snapshot === true && integer(opts.chunk_size)
        && opts.chunk_size >= 1 && opts.chunk_size <= MAX_BATCH_WRITES
      ? opts.chunk_size : MAX_BATCH_WRITES;
    const chunkCount = Math.ceil(ops.length / chunkSize);
    for (let i = 0; i < ops.length; i += chunkSize) {
      ...
      if (opts.snapshot === true) {
        await beforeSnapshotWriteChunk({
          kind: 'snapshot', ref: opts.ref,
          paths: chunk.map((op) => op.ref.path),
          chunk_index: Math.floor(i / chunkSize), chunk_count: chunkCount,
  ...
  function activeTrialControl(config, uid, generation, now) {
    return plain(config) && config.schema === 'personal-live-lab-v2' && config.enabled === true
      && String(config.allowed_uid || '') === String(uid || '')
      && integer(Number(config.generation)) && Number(config.generation) >= 1
      && Number(config.generation) === Number(generation)
      && Number.isSafeInteger(Number(config.activation_auth_time_ms))
      && Number(config.activation_auth_time_ms) > 0
      && Number(config.expires_at_ms || 0) > now;
  }

  function trialAuthValid(user, uid, sid, activationAuthTimeMs) {
    const claims = user && user.customClaims || {};
    const validAfter = Date.parse(String(user && user.tokensValidAfterTime || ''));
    return !!user && user.disabled !== true && user.uid === uid
      && claims.super === true && claims.personal_lab_control === true
      && String(claims.stationId || '') === sid
      && Number.isSafeInteger(Number(activationAuthTimeMs))
      && Number(activationAuthTimeMs) > 0 && Number.isFinite(validAfter)
      && Number(activationAuthTimeMs) >= validAfter;
  }

  function terminalAuthLookupFailure(error) {
    const code = String(error && error.code || '').toLowerCase();
    return code === 'auth/user-not-found' || code === 'user-not-found';
  }

  function deliveryContract(value, runtime, publication, labConfig, now) {
    if (value.delivery_policy === 'live' && value.delivery_allowed === true
        && runtime.mode === MODE.NEW && publication.delivery_policy === 'live'
        && publication.delivery_allowed === true) return 'live';
    if (value.delivery_policy === 'trial_control' && value.delivery_allowed === true
        && runtime.mode === MODE.SHADOW && publication.delivery_policy === 'suppressed_trial'
        && publication.delivery_allowed === false
        && activeTrialControl(labConfig, value.person, value.control_generation, now)
        && Number(value.control_auth_time_ms) === Number(labConfig.activation_auth_time_ms)) {
      return 'trial_control';
    }
    return null;
  }

  function isPreparedLiveOutbox(value, runtime, publication) {
    return value.delivery_policy === 'live' && value.delivery_allowed === true
      && runtime.mode === MODE.SHADOW && publication.delivery_policy === 'live'
      && publication.delivery_allowed === true
      && ['staging', 'complete', 'prepared'].indexOf(publication.status) !== -1;
  }

  async function trialControlFor(sid, notifications) {
    const snap = await liveLabRef(sid).get();
    const config = snap.exists ? (snap.data() || {}) : {};
    const now = Date.parse(clock());
    const uid = String(config.allowed_uid || '');
    const generation = Number(config.generation);
    const activationAuthTimeMs = Number(config.activation_auth_time_ms);
    if (!AUTH_UID_RE.test(uid) || !activeTrialControl(config, uid, generation, now)) return null;
    let user = null;
    try { user = await getAuthUser(uid); }
    catch (error) {
      if (terminalAuthLookupFailure(error)) return null;
      const unavailable = new Error('TRIAL_AUTH_TEMPORARY');
      unavailable.code = 'TRIAL_AUTH_TEMPORARY';
      throw unavailable;
    }
    if (!trialAuthValid(user, uid, sid, activationAuthTimeMs)) return null;
    return (notifications || []).some((item) => item && item.person === uid)
      ? { uid, generation, activation_auth_time_ms: activationAuthTimeMs } : null;
  }
  ...
  function snapshotMaterial(plan, events, people) {
    ...
    return {
      rows, orderedEvents, orderedPeople, peopleCount: peopleById.size,
      absences, absenceCoverage, contentDigest
    };
  }

  async function stageSnapshot(ref, meta, plan, events, people, expectedContentDigest) {
    const material = snapshotMaterial(plan, events, people);
    const {
      rows, orderedEvents, orderedPeople, peopleCount,
      absences, absenceCoverage, contentDigest
    } = material;
    if (nonEmpty(expectedContentDigest) && expectedContentDigest !== contentDigest) {
      throw new ScheduleRuntimeError('draft-recovery-mismatch',
        'לא ניתן לשחזר את טיוטת העריכה בדיוק ולכן היא נעצרה.', 'aborted');
    }
    const ops = [];
    ...
    await commitWrites(ops, {
      snapshot: true, ref, chunk_size: snapshotWriteChunkSize
    });
    ...
    const snapshotMeta = Object.assign({}, meta, {
      ...
      person_count: peopleCount, absence_count: absences.length,
  ...
  async function finalizeDraft(ctx, ref, expectedDigest) {
    try {
      ...
      await db.runTransaction(async (tx) => {
        ...
        if (draft.status === 'complete' && draft.snapshot_complete === true
            && draft.station_id === ctx.sid && draft.content_digest === expectedDigest) return;
  ...
  function completeSheetMonth(month) {
    const from = month + '-01';
    let to = from;
    for (let index = 0; index < 31; index += 1) {
      const next = isoDayOffset(to, 1);
      if (next.slice(0, 7) !== month) break;
      to = next;
    }
  ...
  function requestedSheetInput(data) {
    ...
    if (!hasMatrix && data.label_spans !== undefined) {
      throw new ScheduleRuntimeError('import-label-spans-invalid',
        'גבולות תאים ממוזגים מותרים רק בקובץ Excel.', 'invalid-argument');
    }
    const input = hasMatrix ? data.matrix : data.paste;
    const labelSpans = hasMatrix && data.label_spans !== undefined ? data.label_spans : null;
    const bytes = Buffer.byteLength(JSON.stringify({ input, label_spans: labelSpans }), 'utf8');
    if (bytes > MAX_SHEET_PASTE_BYTES) {
      throw new ScheduleRuntimeError('import-paste-too-large', 'הקלט גדול מדי.', 'resource-exhausted');
    }
    const kind = hasMatrix ? 'matrix' : 'paste';
    return {
      kind, input, labelSpans,
      digest: digest({ kind, input, label_spans: labelSpans })
    };
  }
  ...
  async function sheetImportBasis(ctx, req) {
    ...
    try {
      ...
      parsed = sheetImport.parseSheet(sheet.input, {
        month, policy: projected.policy, label_spans: sheet.labelSpans
      });
  ...
  function sheetImportReport(basis) {
    ...
    return {
      ...
      assignment_absence_conflicts: basis.resolved.assignment_absence_conflicts.map((item) => ({
        uid: item.uid, name: names.get(item.uid) || item.uid, date: item.date,
        stations: item.stations, absences: item.absences
      })),
  ...
  function sheetImportBlockers(basis) {
    ...
    if (counts.assignment_absence_conflicts > 0) out.push('assignment-absence-conflicts');
      ...
      throw new ScheduleRuntimeError('import-blocked',
        blockers.indexOf('no-assignments') !== -1 ? 'לא נמצא אף שיבוץ בהדבקה.'
          : blockers.indexOf('missing-stations') !== -1 || blockers.indexOf('ignored-blocks') !== -1
            ...
            : blockers.indexOf('assignment-absence-conflicts') !== -1
              ? 'יש אדם שמופיע גם בשיבוץ וגם בהיעדרות באותו יום. יש לתקן את הגיליון לפני הייבוא.'
  ...
  /* One projection contract for preview, gap report, edit and publish.  An
   * imported plan is canonical (Eilat/Shahmon/Timna/Yotvata); evaluating it
   * against the raw legacy policy produces a different digest and can show a
  ...
  function projectedPolicyForPlan(policyValue, plan, meta, draftMeta, errorCode) {
    ...
    try {
      ...
      throw new ScheduleRuntimeError(errorCode || 'schedule-station-map-missing',
        'לסידור המיובא חסר מיפוי תחנות למדיניות הפעילה. יש לייבא מחדש.', 'failed-precondition');
    }
  }

  async function editPolicyFor(ctx, policy, active) {
    let map = importedStationMapOf(active.meta, null);
    if (active.plan.imported === true && !map && nonEmpty(active.meta.source_draft_id)) {
  ...
  /* An edited publication keeps the immutable source snapshot of the active
   * publication. `source_id` continues to name the live source used to
   * validate the edit and the gap gate; this field names the source whose
   * roster/events/signature are actually carried by the snapshot. */
  function snapshotSourceIdOf(meta) {
    if (plain(meta) && nonEmpty(meta.snapshot_source_id)) return meta.snapshot_source_id;
  ...
  async function scheduleEditBasis(ctx, req) {
    ...
    const people = source.peopleRaw.filter((person) => person.active === true);
    const snapshotSourceId = snapshotSourceIdOf(active.meta);
    if (!snapshotSourceId) {
      throw new ScheduleRuntimeError('edit-source-basis-missing',
        'לסידור הפעיל חסר מזהה מקור חתום. יש לפרסם סידור חדש לפני עריכה.', 'failed-precondition');
    }
    const snapshotPeople = new Set(active.roster.map((person) => person.id));
    /* New assignments must be both live in the current source and represented
     * in the signed roster that will be preserved. A removal is still allowed
     * for somebody who left the current source; applyEdits has that narrow
     * exception for a person already present in the active plan. */
    const editablePeople = people.filter((person) => snapshotPeople.has(person.id));
    const sourceChanged = snapshotSourceId !== source.id ? {
      from: { source_id: snapshotSourceId, source_digest: active.plan.source_digest },
      current: {
        source_id: source.id,
        source_digest: digest({ source_digest: source.digest, overrides: [] })
      },
    ...
    const effective = {
      snapshot: active.plan.source_snapshot,
  ...
  function stagedEditMatchesBasis(value, basis, fingerprint, expectedSnapshotDigest, report) {
    if (!plain(value) || value.status !== 'staging' || value.station_id !== basis.ctx.sid
        || value.created_by !== basis.ctx.uid || value.request_fingerprint !== fingerprint
        || value.expected_snapshot_digest !== expectedSnapshotDigest
        || value.source_id !== basis.source.id || value.policy_id !== basis.policy.id
        || value.snapshot_source_id !== basis.snapshotSourceId
        || value.base_source_digest !== basis.source.digest
        || value.base_policy_digest !== basis.policy.digest
        || value.source_snapshot !== basis.plan.source_snapshot
        || value.source_version !== basis.plan.source_version
        || value.contract_station_id !== basis.plan.contract_station_id
        || value.source_revision !== basis.plan.source_revision
        || value.source_digest !== basis.plan.source_digest
        || value.policy_version !== basis.plan.policy_version
        || value.policy_digest !== basis.plan.policy_digest
        || value.from !== basis.plan.from || value.to !== basis.plan.to
        || value.edit_digest !== basis.editDigest) return false;
    return stable(value.edit_base) === stable(basis.base)
      && stable(value.station_map || null) === stable(basis.editPolicy.station_map || null)
      && stable(value.summary) === stable(basis.plan.summary)
      && stable(value.edit_report) === stable(report);
  }

  async function cancelUnrecoverableEdit(ref, error) {
    if (!(error instanceof ScheduleRuntimeError)
        || ['schedule-mode-blocked', 'edit-no-active', 'edit-base-stale'].indexOf(error.code) === -1) return;
    await cancelStagedSnapshot(ref, 'edit-recovery-' + error.code);
  }
  ...
  async function applyScheduleEdit(req) {
    ...
    if (existing.exists) {
      ...
      if (before.status === 'complete') {
        ...
        return {
          ...
          notified_people: integer(published.notified_people) ? published.notified_people : 0,
          report: plain(before.edit_report) ? before.edit_report : null
        };
      }
      if (before.status !== 'staging') {
        throw new ScheduleRuntimeError('draft-staging-invalid',
          'טיוטת העריכה נמצאת במצב שאינו ניתן לשחזור.', 'aborted');
      }
      stagedDraft = before;
    }
    let basis;
    try {
      basis = await scheduleEditBasis(ctx, req);
    } catch (error) {
      if (stagedDraft) await cancelUnrecoverableEdit(ref, error);
      throw error;
    }
    ...
    if (!expectedDigest || expectedDigest !== basis.editDigest) {
      if (stagedDraft) await cancelStagedSnapshot(ref, 'edit-recovery-report-stale');
    ...
    if (!basis.applied.changes.length) {
      if (stagedDraft) await cancelStagedSnapshot(ref, 'edit-recovery-no-changes');
      throw new ScheduleRuntimeError('edit-no-changes', 'העריכה אינה משנה דבר בסידור הפעיל.', 'failed-precondition');
    }
    const expectedSnapshotDigest = snapshotMaterial(plan, effective.events, effective.roster).contentDigest;
    ...
    report.report_bytes = requireEditReportSize(report);
    await db.runTransaction(async (tx) => {
      if (stagedDraft) recoveryFailure = null;
      ...
      if (!monthAuthorityEnabled && !editBaseMatches(snaps[3].exists ? (snaps[3].data() || {}) : null, base)) {
        ...
        tx.update(ref, {
          status: 'cancelled', cancel_reason: 'edit-recovery-edit-base-stale',
          cancelled_at: FV.serverTimestamp()
        });
        return;
      }
      if (stagedDraft) {
        const liveDraft = snaps[2].exists ? (snaps[2].data() || {}) : null;
        if (!stagedEditMatchesBasis(
          liveDraft, basis, fingerprint, expectedSnapshotDigest, report
        )) {
          recoveryFailure = new ScheduleRuntimeError('draft-recovery-mismatch',
            'לא ניתן לשחזר את טיוטת העריכה בדיוק. הטיוטה בוטלה ויש להתחיל פעולה חדשה.', 'aborted');
          tx.update(ref, {
            status: 'cancelled', cancel_reason: 'edit-recovery-mismatch',
            cancelled_at: FV.serverTimestamp()
          });
          return;
        }
    ...
    const contentDigest = await stageSnapshot(
      ref, {}, plan, effective.events, effective.roster, expectedSnapshotDigest
    );
    await finalizeDraft(ctx, ref, contentDigest);
  ...
  async function getQualificationCatalog(req) {
    ...
    return {
      ...
      unknown_holders: Array.from(holdings.entries())
        .filter(([uid, held]) => !known.has(uid) && held.qualifications.length > 0)
        .map(([uid]) => uid).sort()
    };
  }
  ...
  async function saveQualification(req) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      result = { key: next.key, revision: nextRevision };
      tx.set(docRef, Object.assign({}, next, {
        station_id: ctx.sid, revision: nextRevision,
    ...
    return { duplicate, key: result.key, revision: Number(result.revision) };
  }
  ...
  async function deleteQualification(req) {
    ...
    // Read the revision fence *before* the holders query. Every holdings write
    // increments it in the same transaction. Reading it afterwards would let
    // a holder added between the query and this read become part of the fence
  ...
  async function setPersonQualifications(req) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      requireLiveManager(snaps[0], snaps[1], ctx);
      if (snaps[4].exists) {
        const prior = snaps[4].data() || {};
        if (prior.fingerprint !== fingerprint) {
          throw new ScheduleRuntimeError('request-conflict', 'אותו מזהה פעולה כבר שימש לשינוי אחר.', 'already-exists');
        }
        duplicate = true;
        result = plain(prior.result) ? prior.result : (snaps[3].data() || {});
        return;
      }
      const person = snaps[2].exists ? (snaps[2].data() || {}) : null;
      const live = snaps[3].exists ? (snaps[3].data() || {}) : null;
      const liveRevision = live && Number.isInteger(live.revision) ? live.revision : 0;
      const activePerson = scheduleAccess.activeMember(person, ctx.sid);
      /* הקצאה מותרת רק לחבר פעיל. ניקוי הוא מסלול צר ובטוח שגם מאפשר
       * להסיר החזקה של מי שעזב/הושבת: חייבת להיות רשומת החזקה קיימת,
       * והרשימה המבוקשת חייבת להיות ריקה. כך אין דרך להקצות כשירות
       * למזהה שכבר אינו חבר, אך כשירות מותאמת לא נשארת נעולה לעד. */
      ...
      const nextRevision = liveRevision + 1;
      result = { qualifications: next, revision: nextRevision };
      /* גם ניקוי נשמר כרשומת tombstone עם revision עולה. מחיקת המסמך
       * הייתה מחזירה את ה-CAS ל-revision 0 ומאפשרת ABA: לקוח ישן היה
       * יכול להקצות מחדש על בסיס 0 אחרי הקצאה וניקוי. */
      tx.set(personRef, {
      ...
      tx.create(auditRef, Object.assign({
        ...
        before, after: next, result, by: ctx.uid, at: FV.serverTimestamp()
  ...
  const MAX_GAP_REPORT_DAYS = 31;
  const MAX_GAP_CANDIDATES = 8;
  // Gap decisions are an operational safety gate, so an immutable schedule
  ...
  async function loadLiveGapPeople(ctx, sourcePeople, read) {
    const known = Array.isArray(sourcePeople) ? sourcePeople : [];
    if (known.length > MAX_GAP_LIVE_ROSTER) {
    ...
    // and can never satisfy a staffing or qualification minimum.
    return known.map((person) => Object.assign({}, person, {
      active: person.active !== false && activeOperationalMember(live.get(person.id), ctx.sid)
    }));
  }

  /* `read` אופציונלי — בתוך עסקת הפרסום הקטלוג, המחזיקים, מדיניות הפער
   * **והסגל החי** נקראים דרך `tx.get`, כך שהשער מחושב על מה שהעסקה
   * רואה ולא על מה שנקרא לפניה (§1 · TOCTOU). */
  async function gapContext(ctx, config, knownPeople, read) {
    let sourcePeople = Array.isArray(knownPeople) ? knownPeople : [];
  ...
  function gapBasisDigest(gapCtx) {
    const catalog = gapCtx.catalog.map((entry) => ({
      key: entry.key, label: entry.label, active: entry.active, minimum: entry.minimum,
      critical: entry.critical, order: entry.order, revision: entry.revision || 0
    })).sort((a, b) => compareCanonical(a.key, b.key));
    const holdings = Array.from(gapCtx.holdings.entries()).map(([uid, held]) => ({
      uid,
      qualifications: (Array.isArray(held.qualifications) ? held.qualifications : []).slice().sort(compareCanonical),
      revision: Number(held.revision || 0)
    })).sort((a, b) => compareCanonical(a.uid, b.uid));
    const people = gapCtx.people.map((person) => ({
      id: person.id,
      name: String(person.full_name || person.name || person.id),
      sub_station: person.sub_station || null,
      active: person.active === true
    })).sort((a, b) => compareCanonical(a.id, b.id));
    return digest({ catalog, holdings, people,
      station_minimum: gapCtx.station_minimum,
      gap_policy_revision: gapCtx.gap_policy_revision });
  }

  function gapReportFor(gapCtx, policyValue, plan) {
    try {
      const report = scheduleGaps.analyzeGaps({
      ...
      // An acknowledgement is not merely approval of the visible counts. Bind
      // it to the canonical live-roster/qualification/policy basis as well, so
      // a membership change after preview can never reuse an old approval even
      // when the numeric list of non-critical gaps happens to stay identical.
      if (report.acknowledgeable.length) {
  ...
  function gapSummaryFor(report) {
    const bounded = (entry) => Object.assign({}, entry, Array.isArray(entry.candidates)
      ? { candidates: entry.candidates.slice(0, MAX_GAP_CANDIDATES), candidates_truncated: entry.candidates.length > MAX_GAP_CANDIDATES }
      : {});
    return {
      summary: report.summary,
      candidates_basis: report.candidates_basis,
      candidates_note: report.candidates_note,
      blocking: report.blocking.slice(0, MAX_GAP_DETAIL).map(bounded),
      acknowledgeable: report.acknowledgeable.slice(0, MAX_GAP_DETAIL).map(bounded),
      truncated: report.blocking.length > MAX_GAP_DETAIL || report.acknowledgeable.length > MAX_GAP_DETAIL,
      digest: report.digest
    };
  }

  function boundedGapDays(days) {
    return days.slice(0, MAX_GAP_REPORT_DAYS).map((day) => Object.assign({}, day, {
      station_candidates: (day.station_candidates || []).slice(0, MAX_GAP_CANDIDATES),
      qualifications: (day.qualifications || []).map((entry) => Object.assign({}, entry, {
        candidates: (entry.candidates || []).slice(0, MAX_GAP_CANDIDATES)
      }))
    }));
  }
  ...
  async function getGapReport(req) {
    ...
    return Object.assign({
      ...
      station_minimum: gapCtx.station_minimum,
      gap_policy_revision: gapCtx.gap_policy_revision,
      days: boundedGapDays(report.days),
      days_total: report.days.length,
      days_truncated: report.days.length > MAX_GAP_REPORT_DAYS
    }, gapSummaryFor(report));
  }
  ...
  async function saveGapPolicy(req) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (snaps[3].exists) {
        ...
        duplicate = true;
        result = plain(prior.result)
          ? prior.result
          : { station_minimum: minimum, revision: expectedRevision + 1 };
        return;
      }
      ...
      result = { station_minimum: minimum, revision: liveRevision + 1 };
      tx.set(gapPolicyRef(ctx.sid), { station_id: ctx.sid, station_minimum: minimum, revision: result.revision, updated_by: ctx.uid, updated_at: FV.serverTimestamp() });
      tx.create(auditRef, {
        ...
        after: { station_minimum: minimum }, result, by: ctx.uid, at: FV.serverTimestamp()
      });
    });
    return { duplicate, station_minimum: result.station_minimum, revision: Number(result.revision) };
  }
  ...
  async function getDraftPreview(req) {
    ...
    const gapReport = gapReportFor(gapCtx, previewPolicy.value, fullSnapshot.plan);
    await requireLiveManagerNow(ctx);
    return {
      draft_id: draftId,
  ...
  async function setScheduleDisplay(req) {
    ...
    // A completed receipt is authoritative for an identical retry.  Read it
    // before touching the draft, runtime mode or display pointer so cleanup or
    // a later mode change cannot turn a successful request into a false error.
    // Live authorization is still re-read in this transaction; revocation wins.
    let replay = null;
    await db.runTransaction(async (tx) => {
      const snaps = await Promise.all([
        tx.get(liveUserRef(ctx.sid, ctx.uid)),
      ...
      requireLiveManager(snaps[0], snaps[1], ctx);
      if (!snaps[2].exists) return;
      const operation = snaps[2].data() || {};
      if (operation.request_fingerprint !== fingerprint || !plain(operation.result)) {
        throw new ScheduleRuntimeError('request-conflict',
          'אותו מזהה פעולה כבר שימש לבקשה אחרת.', 'already-exists');
      }
      replay = Object.assign({}, operation.result, { duplicate: true });
    });
    if (replay) return replay;

    let prepared = null;
    if (action === 'show') {
      const requiredRange = completeSheetMonth(month);
      const ref = stationRef(ctx.sid).collection('schedule_drafts').doc(draftId);
      const snap = await ref.get();
      const meta = snap.exists ? (snap.data() || {}) : {};
      if (!snap.exists || meta.status !== 'complete' || meta.snapshot_complete !== true
          ...
          || meta.from !== requiredRange.from || meta.to !== requiredRange.to) {
        throw new ScheduleRuntimeError('display-draft-invalid',
          'אפשר להציג בלוח רק טיוטת ייבוא חתומה שמכסה את כל החודש שנבחר.');
      }
    ...
    await db.runTransaction(async (tx) => {
      ...
      // A concurrent first writer may have completed after the replay probe.
      // Preserve replay-first semantics inside the committing transaction too.
      if (operation) {
        if (operation.request_fingerprint !== fingerprint || !plain(operation.result)) {
          throw new ScheduleRuntimeError('request-conflict',
            'אותו מזהה פעולה כבר שימש לבקשה אחרת.', 'already-exists');
      ...
      if (prepared) {
        ...
        if (liveDraft.status !== 'complete' || liveDraft.snapshot_complete !== true
            ...
            || liveDraft.from !== requiredRange.from || liveDraft.to !== requiredRange.to
  ...
  async function displaySnapshotForRange(ctx, config, range, state) {
    ...
    const requiredRange = completeSheetMonth(state.month);
    if ((config.mode !== MODE.OFF && config.mode !== MODE.SHADOW)
        ...
        || state.from !== requiredRange.from || state.to !== requiredRange.to
    ...
    if (meta.policy_id !== config.active_policy_id || meta.source_id !== config.active_source_id) {
      throw new ScheduleRuntimeError('display-draft-stale',
        'הסידור שנבחר אינו מבוסס על חוקי התחנה ומקור כוח האדם הפעילים.', 'aborted');
    }
  ...
  async function activePublicationGate(ref) {
    return db.runTransaction(async (tx) => {
      ...
      if (runtime.mode === MODE.SHADOW && publication.delivery_policy === 'suppressed_trial'
          && publication.delivery_allowed === false) {
        const labSnap = await tx.get(liveLabRef(stationId));
        const lab = labSnap.exists ? (labSnap.data() || {}) : {};
        if (activeTrialControl(lab, lab.allowed_uid, lab.generation, Date.parse(clock()))) {
        return { kind: 'trial_control', allowed_uid: String(lab.allowed_uid),
          generation: Number(lab.generation),
          activation_auth_time_ms: Number(lab.activation_auth_time_ms) };
        }
      }
      return null;
    });
  ...
  async function releaseOutbox(ref) {
    ...
    let released = 0;
    while (true) {
      const gate = await activePublicationGate(ref);
      if (!gate) return { released };
      const snap = await ref.collection('schedule_outbox')
        .where('status', '==', 'blocked').limit(100).get();
      if (snap.empty) return { released };
        ...
        current.forEach((item,index) => {
          ...
          const allowed = gate.kind === 'live'
            ...
            : value.delivery_allowed === true && value.delivery_policy === 'trial_control'
              && value.person === gate.allowed_uid
              && Number(value.control_generation) === gate.generation
              && Number(value.control_auth_time_ms) === gate.activation_auth_time_ms;
          if (!allowed) {
  ...
  async function reconcileOutbox(ref, now) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (value.delivery_policy === 'trial_control') refs.push(liveLabRef(stationId));
      ...
      const lab = checks[4] && checks[4].exists ? (checks[4].data() || {}) : {};
      const contract = deliveryContract(value, runtime, publication, lab, now);
      const preparedLive = isPreparedLiveOutbox(value, runtime, publication);
      const preActivationLive = (contract === 'live' || preparedLive)
        && ['staging', 'complete', 'prepared'].indexOf(publication.status) !== -1;
      if (!contract && !preparedLive) {
  ...
  async function promoteToNew(req) {
    ...
    // Gap inputs can change after a publication was prepared (qualification
    // catalog, holdings, active roster or the station minimum).  Load the
    // immutable candidate snapshot/source now; their signed metadata is read
    // again in the transaction below.  Mutable gap inputs are read in that
    // transaction and must match the preparation receipt exactly.
    const candidateSnapshot = await readSnapshot(pubRef, candidate.meta, null);
    const candidatePolicy = await loadPolicy(ctx, candidate.meta.policy_id);
    const candidateSource = await loadSource(ctx, candidate.meta.source_id);
    const candidateGapPeople = candidateSource.peopleRaw.filter((person) => person.active === true);
    const candidateGapPolicy = projectedPolicyForPlan(
      candidatePolicy.value, candidateSnapshot.plan, candidate.meta, null,
      'gap-station-map-missing'
    ).value;
    ...
    const outcome = await db.runTransaction(async (tx) => {
      ...
      const preparedGap = plain(livePub.gap_report) ? livePub.gap_report : {};
      if (!nonEmpty(preparedGap.basis_digest)
          || preparedGap.basis_digest !== gapBasisDigest(liveGapCtx)
          || preparedGap.digest !== liveGapReport.digest) {
        throw new ScheduleRuntimeError('cutover-gaps-changed',
          'הכשירויות, הסגל או קווי המינימום השתנו מאז הכנת הסידור. יש להכין ולבדוק אותו מחדש.',
          'aborted');
      }
      // The original acknowledgement is bound to the exact report digest.
      // Reusing a boolean here would silently approve a different list.
      requireGapClearance(liveGapReport, String(preparedGap.acknowledgement || ''));
  ...
  async function publish(req) {
    ...
    const requestCoreFingerprint = digest({
      contract_version: 3, station_id: ctx.sid, actor_uid: ctx.uid,
      request_id: requestId, intent: 'publish', draft_id: draftId,
    ...
    const requestFingerprint = digest({
      ...
      request_id: requestId, intent: 'publish', draft_id: draftId,
      expected_content_digest: expectedContentDigest,
      gap_acknowledgement: gapAcknowledgement || null
    });
    const completedReplay = await pubRef.get();
    if (completedReplay.exists) {
      const completed = completedReplay.data() || {};
      if (completed.request_contract_version === 3 && completed.status === 'active'
          && plain(completed.operation_receipt)
          && completed.request_fingerprint !== requestFingerprint) {
        throw new ScheduleRuntimeError('publication-conflict',
          'מזהה הפרסום כבר קיים עם תוכן אחר.', 'already-exists');
      }
      if (completed.request_contract_version === 3 && completed.status === 'active'
          && completed.station_id === ctx.sid && completed.request_id === requestId
          && completed.source_draft_id === draftId && completed.published_by === ctx.uid
          && completed.content_digest === expectedContentDigest
          && plain(completed.operation_receipt)) {
        await requireLiveManagerNow(ctx);
        const replayPair = await Promise.all([runtimeRef(ctx.sid).get(), activeRef(ctx.sid).get()]);
    ...
    if (existing.exists) {
      ...
      const fingerprintForExisting = existingData.request_contract_version === 3
        ? (existingData.status === 'staging' ? requestCoreFingerprint : requestFingerprint) : digest({
          ...
          revision: pointsToExisting ? Number(existingData.revision) : revision,
          previous_publication_id: pointsToExisting
            ? (nonEmpty(existingData.previous_publication_id)
              ? existingData.previous_publication_id : null) : expectedPrevious,
          content_hash: planned.publication.content_hash
        });
      const storedFingerprintForExisting = existingData.request_contract_version === 3
          && existingData.status === 'staging'
        ? existingData.request_core_fingerprint : existingData.request_fingerprint;
      if (storedFingerprintForExisting !== fingerprintForExisting) {
        throw new ScheduleRuntimeError('publication-conflict',
          'מזהה הפרסום כבר קיים עם תוכן אחר.', 'already-exists');
      }
      if (pointsToExisting) {
        ...
        // edit drafts created by the previous release can replay their signed
        // report as a compatibility fallback.
        const replayNotified = integer(existingData.notified_people)
            && existingData.notified_people >= 0
          ? existingData.notified_people
          : (plain(draftMeta.edit_report) && integer(draftMeta.edit_report.notifications)
        ...
        return {
          ...
          trial_control_notifications: integer(existingData.trial_control_notifications)
            ? existingData.trial_control_notifications : 0,
   ...
   * עוצרים את אחראי הסידור. שערי מבנה, זהות והרשאה נשארים fail-closed. */
    ...
    if (!existing.exists) await pubRef.create({
      station_id: ctx.sid, status: 'staging', request_id: requestId,
      request_fingerprint: requestFingerprint,
      request_core_fingerprint: requestCoreFingerprint,
      ...
      snapshot_source_id: draftMeta.edited === true
        ? draftMeta.snapshot_source_id : draftMeta.source_id,
      source_draft_id: draftId,
      previous_publication_id: before ? before.pointer.publication_id : null,
      created_at: FV.serverTimestamp(),
    ...
    const operationReceipt = {
      ...
      trial_control_notifications: trial && trialControl ? 1 : 0,
      suppressed_notifications: trial
        ? planned.notifications.length - (trialControl ? 1 : 0) : 0,
      blocked_notifications: 0, summary: next.plan.summary
    };
    const outboxOps = planned.notifications.map((notification) => {
      const control = !!(trialControl && notification.person === trialControl.uid);
      return {
        ...
        data: {
          ...
          changed_by: ctx.uid, attempt: 0,
          delivery_allowed: !trial || control,
          delivery_policy: trial ? (control ? 'trial_control' : 'suppressed_trial') : 'live',
          control_generation: control ? trialControl.generation : null,
          control_auth_time_ms: control ? trialControl.activation_auth_time_ms : null,
          status: trial && !control ? 'suppressed_trial' : 'blocked',
          suppressed_at: trial && !control ? FV.serverTimestamp() : null,
          expires_at: new Date(Date.parse(clock()) + OUTBOX_TTL_MS),
          created_at: FV.serverTimestamp()
        }
      };
    ...
    try {
      ...
      tx.create(stationRef(ctx.sid).collection('schedule_audit').doc('a_' + randomId()), Object.assign({
        ...
        source_changed: plain(liveDraft.edit_summary) ? liveDraft.edit_summary.source_changed || null : null
      } : {}));
      });
    } catch (error) {
  ...
  async function rollback(req) {
    ...
    const gapAcknowledgement = String(data.gap_acknowledgement || '');
    /* The fingerprint is the normalized request itself, so it can be checked
     * before consulting the now-moved active pointer. In particular, a retry
     * cannot substitute another gap acknowledgement or rollback intent. */
    const requestFingerprint = digest({
      contract_version: 3, station_id: ctx.sid, actor_uid: ctx.uid,
      request_id: requestId, intent: 'rollback',
    ...
    const pubRef = stationRef(ctx.sid).collection('schedule_publications').doc(pubId);
    const replayReceipt = (publication, active) => {
      if (publication.request_contract_version === 3 && plain(publication.operation_receipt)
          && publication.station_id === ctx.sid && publication.status === 'active'
          && publication.operation === 'rollback' && publication.request_id === requestId
          && publication.published_by === ctx.uid
          && publication.rollback_from_publication_id === expectedActive
          && publication.rollback_target_publication_id === targetId
          && publication.rollback_reason_code === reasonCode) {
        return Object.assign({ duplicate: true }, publication.operation_receipt);
      }
      if (!plain(publication) || !plain(active)
          || publication.station_id !== ctx.sid || publication.status !== 'active'
          || publication.operation !== 'rollback' || publication.request_id !== requestId
          || publication.published_by !== ctx.uid
          || publication.rollback_from_publication_id !== expectedActive
          || publication.rollback_target_publication_id !== targetId
          || publication.rollback_reason_code !== reasonCode
          || active.publication_id !== pubId
          || Number(active.revision) !== Number(publication.revision)
          || active.content_digest !== publication.content_digest
          || !integer(publication.notified_people) || publication.notified_people < 0) {
        throw new ScheduleRuntimeError('rollback-conflict',
          'קבלת החזרה השמורה אינה מלאה או אינה תואמת לבקשה.', 'already-exists');
      }
      return {
        ...
        trial_control_notifications: integer(publication.trial_control_notifications)
          ? publication.trial_control_notifications : 0,
    ...
    const replaySnap = await pubRef.get();
    const replayPub = replaySnap.exists ? (replaySnap.data() || {}) : {};
    if (replaySnap.exists && replayPub.request_contract_version === 3) {
      if (replayPub.request_fingerprint !== requestFingerprint) {
        throw new ScheduleRuntimeError('rollback-conflict',
          'מזהה החזרה כבר שימש לפעולה אחרת.', 'already-exists');
      }
      if (replayPub.status === 'active' && plain(replayPub.operation_receipt)) {
        const receipt = replayReceipt(replayPub, {});
        await requireLiveManagerNow(ctx);
        const replayPair = await Promise.all([runtimeRef(ctx.sid).get(), activeRef(ctx.sid).get()]);
        const replayRuntime = replayPair[0].exists ? (replayPair[0].data() || {}) : {};
        const replayPointer = replayPair[1].exists ? (replayPair[1].data() || {}) : {};
    ...
    const legacyRollbackFingerprint = digest({
      station_id: ctx.sid, uid: ctx.uid, request_id: requestId, intent: 'rollback',
      delivery_policy: trial ? 'suppressed_trial' : 'live',
      expected_active_publication_id: expectedActive,
      target_publication_id: targetId, reason_code: reasonCode,
      gap_acknowledgement: gapAcknowledgement || null
    });
    const expectedStoredFingerprint = firstPub.request_contract_version === 3
      ? requestFingerprint : legacyRollbackFingerprint;
    if (firstPair[0].exists && firstPub.request_fingerprint !== expectedStoredFingerprint) {
      throw new ScheduleRuntimeError('rollback-conflict',
        'מזהה החזרה כבר שימש לפעולה אחרת.', 'already-exists');
    }
    ...
    const earlyRollbackGapCtx = await gapContext(ctx, config, rollbackGapPeople);
    const earlyRollbackGapReport = gapReportFor(earlyRollbackGapCtx, rollbackEffectivePolicy, target.plan);
    requireGapClearance(earlyRollbackGapReport, gapAcknowledgement);
    ...
    if (existing.exists) {
      const existingData = existing.data() || {};
      const active = await activeRef(ctx.sid).get();
      const expectedExistingFingerprint = existingData.request_contract_version === 3
        ? requestFingerprint : legacyRollbackFingerprint;
    ...
    const trialControl = trial ? await trialControlFor(ctx.sid, planned.notifications) : null;
    const operationReceipt = {
      publication_id: pubId, revision, rolled_back_to: targetId,
      notified_people: trial ? 0 : planned.notifications.length,
      trial_control_notifications: trial && trialControl ? 1 : 0,
      suppressed_notifications: trial
        ? planned.notifications.length - (trialControl ? 1 : 0) : 0,
    ...
    const outboxOps = planned.notifications.map((notification) => {
      ...
      data: {
        ...
        control_generation: control ? trialControl.generation : null,
        control_auth_time_ms: control ? trialControl.activation_auth_time_ms : null,
        status: trial ? (control ? 'blocked' : 'suppressed_trial') : 'blocked',
        suppressed_at: trial && !control ? FV.serverTimestamp() : null,
        expires_at: new Date(Date.parse(clock()) + OUTBOX_TTL_MS),
        created_at: FV.serverTimestamp()
      }
    ...
    try {
      ...
      const rollbackGapReport = gapReportFor(txRollbackGapCtx, rollbackEffectivePolicy, target.plan);
      requireGapClearance(rollbackGapReport, gapAcknowledgement);
      const rollbackGapRecord = {
        summary: rollbackGapReport.summary,
  ...
  function legacyDayBlock(day, viewer, events, primaryCrew) {
    ...
    return {
      ...
      // היא באה ממחזור A/B/C המאומת, לא מרשימת האנשים: כוננות יכולה
      // להוסיף צוות נוסף ורשימת הסגל יכולה להיות ריקה בלי לשנות את היממה.
      crew: primaryCrew,
  ...
  function rosterCrewsFromDocs(docs) {
    const out = new Map();
    docs.forEach((doc) => {
  ...
  async function legacyDisplayBasis(ctx, range) {
    const root = stationRef(ctx.sid);
    const reads = await Promise.all([
      root.collection('roster').limit(MAX_LEGACY_ROSTER + 1).get(),
      root.collection('rotations').limit(MAX_LEGACY_ROTATIONS + 1).get(),
      range.dates.length
        ? db.getAll.apply(db, range.dates.map((date) => root.collection('shift_overrides').doc(date)))
        : []
    ]);
    if (reads[0].size > MAX_LEGACY_ROSTER) {
      throw new ScheduleRuntimeError('legacy-roster-too-large',
    ...
    const rosterDocs = reads[0].docs.slice().sort((left, right) => compareCanonical(left.id, right.id));
    const rotationDocs = reads[1].docs.slice().sort((left, right) => compareCanonical(left.id, right.id));
    const overrideDocs = new Map();
    reads[2].filter((doc) => doc && doc.exists).forEach((doc) => overrideDocs.set(doc.id, doc));
    if (overrideDocs.size > MAX_LEGACY_OVERRIDES) {
      throw new ScheduleRuntimeError('legacy-overrides-too-large',
    ...
    const pairs = (docs) => docs.map((doc) => [doc.id, doc.data() || {}]);
    const overrides = Array.from(overrideDocs.values())
      .sort((left, right) => compareCanonical(left.id, right.id));
    return Object.freeze({
      range: Object.freeze({ from: range.from, to: range.to }),
      rosterDocs: Object.freeze(rosterDocs),
      rotationDocs: Object.freeze(rotationDocs),
      overrideDocs,
      displayDigest: digest({
        range: { from: range.from, to: range.to },
        roster: pairs(rosterDocs),
        rotations: pairs(rotationDocs),
        overrides: pairs(overrides)
      })
    });
  }

  async function requireSameLegacyDisplayBasis(ctx, basis, range) {
    const now = await legacyDisplayBasis(ctx, range);
    if (now.displayDigest !== basis.displayDigest) {
      throw new ScheduleRuntimeError('legacy-schedule-changed',
        'הסידור הקיים השתנה בזמן הקריאה. יש לרענן.', 'aborted');
    }
  }

  async function legacyDisplayCrews(ctx, from, to) {
    const dates = [];
    let cursor = isoDayOffset(from, 0);
    const last = isoDayOffset(to, 0);
    while (cursor <= last) {
      dates.push(cursor);
    ...
    const range = Object.freeze({ from: dates[0], to: dates[dates.length - 1], dates });
    // This display-only basis deliberately excludes approved swaps.  Swaps
    // affect who works, but never which A/B/C crew owns the 24-hour column.
    // Effective/personal schedule readers keep their full signed basis.
    const basis = await legacyDisplayBasis(ctx, range);
    try {
      const projectedOverrides = Array.from(basis.overrideDocs.values()).map((doc) => {
        const value = doc.data() || {};
        // Preserve the established runtime contract: the document id is the
        // only authoritative day.  A stale payload date is a specific data
        // integrity failure, not a recoverable compatibility warning.
        if (value.date !== undefined && value.date !== null && value.date !== ''
            && value.date !== doc.id) {
          throw new ScheduleRuntimeError('legacy-override-date',
            'חריג סידור מכיל תאריך סותר ולכן אינו מוצג.', 'failed-precondition');
      ...
      // Reuse the strict legacy contract: one complete A/B/C cycle, canonical
      // dates and only supported override shapes.  The display must not turn a
      // malformed rotation into a neutral-looking "unknown" chip.
      const compatible = legacyCompatibility.projectLegacyScheduleCompatibility({
        mode: MODE.OFF,
        rotations: basis.rotationDocs.map((doc) => ({ id: doc.id, value: doc.data() || {} })),
        overrides: projectedOverrides
      });
      if (compatible.warnings.length) {
        throw new ScheduleRuntimeError('effective-schedule-invalid',
          'חריג משמרת פגום מונע הצגה של צוות יממתי שגוי.', 'failed-precondition');
      }
      // The header names the one primary 24-hour crew.  A swap replaces it;
      // standby/holiday/training do not.  It is deliberately independent of
      // how many members of that crew happen to be active in the roster.
      const projection = operationalProjection.createOperationalProjection({
        source: operationalProjection.SOURCE.LEGACY,
        station_id: ctx.sid,
        roster: [],
        legacy: {
          rotations: compatible.rotations,
          overrides: compatible.overrides,
          approved_swaps: []
        }
      });
      const dayCrews = new Map(dates.map((date) => [date, projection.primaryCrewOn(date)]));
      return Object.freeze({
        basis,
        rosterCrew: rosterCrewsFromDocs(basis.rosterDocs),
        dayCrews,
        verify: () => requireSameLegacyDisplayBasis(ctx, basis, range)
      });
    } catch (error) {
      if (error instanceof ScheduleRuntimeError) throw error;
      throw new ScheduleRuntimeError('effective-schedule-invalid',
        'מחזור המשמרות הקיים חסר או פגום ולכן הלוח אינו מוצג.', 'failed-precondition');
    }
  }
  ...
  function legacyStationView(ctx, window, date, dayCrews) {
    const byDate = new Map((window.days || []).map((day) => [day.date, day]));
    const primaryCrew = (day) => dayCrews.get(day);
    return {
      ...
      }, ctx.uid, window.events, primaryCrew(isoDayOffset(date, -1))),
  ...
  function requestedStationRange(req) {
    const data = req && req.data === undefined ? {} : (req && req.data);
    if (!plain(data) || Object.keys(data).some((key) =>
      key !== 'from' && key !== 'to' && key !== 'display_imported')) {
      throw new ScheduleRuntimeError('station-range-input',
        'תצוגת הטווח מקבלת התחלה, סיום ובחירת תצוגת ייבוא בלבד.', 'invalid-argument');
    }
    if (data.display_imported !== undefined && typeof data.display_imported !== 'boolean') {
      throw new ScheduleRuntimeError('station-range-input',
        'בחירת תצוגת הייבוא חייבת להיות ערך כן/לא.', 'invalid-argument');
    }
    const from = isoDayOffset(String(data.from || ''), 0);
    const to = isoDayOffset(String(data.to || ''), 0);
    if (from > to) {
    ...
    return Object.freeze({
      from, to, dates: Object.freeze(dates), displayImported: data.display_imported === true
    });
  }
  ...
  async function requireDisplayConfigurationUnchanged(ctx, config) {
    const after = await configuration(ctx.sid);
    if (after.mode !== config.mode
        || after.active_policy_id !== config.active_policy_id
        || after.active_source_id !== config.active_source_id) {
      throw new ScheduleRuntimeError('display-config-changed',
        'מצב הסידור, חוקי התחנה או מקור כוח האדם השתנו בזמן הקריאה. יש לרענן.', 'aborted');
    }
  }
  ...
  async function getStationRange(req) {
    ...
    if (config.mode === MODE.OFF) {
      ...
      if (displayed) {
        const sidecar = await readLiveGuardProjection(ctx, range.dates);
        const service = serviceFor(ctx);
        const displayCrews = await legacyDisplayCrews(ctx, range.from, range.to);
        const extras = {
          ...
          rosterCrew: displayCrews.rosterCrew,
          dayCrews: displayCrews.dayCrews
        };
      ...
      await beforeLiveGuardViewRecheck({ kind: 'legacy-display', ctx, mode: config.mode });
      await displayCrews.verify();
      await requireModeUnchanged(ctx, config);
      await requireLiveBoardViewer(ctx);
      return {
        ...
        days: range.dates.map((date) => legacyDayBlock(
          ...
          displayCrews.dayCrews.get(date)))
      };
    }
    ...
    if (!active) {
      ...
      await displayCrews.verify();
      await requireModeUnchanged(ctx, config);
      await requireNoActivePublication(ctx);
      await requireLiveBoardViewer(ctx);
      return {
  ...
  async function getStation(req) {
    ...
    if (config.mode === MODE.OFF) {
      ...
      await requireModeUnchanged(ctx, config);
      await requireLiveBoardViewer(ctx);
      return legacyStationView(ctx, window, date, displayCrews.dayCrews);
    }
    ...
    if (!active) {
      ...
      await requireNoActivePublication(ctx);
      await requireLiveBoardViewer(ctx);
      return Object.assign(legacyStationView(ctx, window, date, displayCrews.dayCrews),
        { mode: config.mode, fallback: 'legacy' });
    }
  ...
  async function validateOutboxForSend(ref, leaseToken, claimed) {
    ...
    if (candidateValue.delivery_policy === 'trial_control') {
      try { trialAuth = await getAuthUser(String(candidateValue.person || '')); }
      catch (error) {
    ...
    return db.runTransaction(async (tx) => {
      ...
      if (contract === 'trial_control') {
        if (trialAuthTerminal || !trialAuthValid(trialAuth, person, stationId,
          Number(value.control_auth_time_ms))) {
          cancelOutbox(tx, ref, 'trial-control-inactive');
          return false;
        }
      }
  ...
  async function deliverOutbox(ref) {
    ...
    await db.runTransaction(async (tx) => {
      ...
      const refs = [runtimeRef(stationId), activeRef(stationId), publicationRef,
        liveUserRef(stationId, person)];
      if (data.delivery_policy === 'trial_control') refs.push(liveLabRef(stationId));
      const checks = await Promise.all(refs.map((item) => tx.get(item)));
      const runtime = checks[0].exists ? (checks[0].data() || {}) : {};
      const pointer = checks[1].exists ? (checks[1].data() || {}) : {};
      const publication = checks[2].exists ? (checks[2].data() || {}) : {};
      const lab = checks[4] && checks[4].exists ? (checks[4].data() || {}) : {};
      if (!deliveryContract(data, runtime, publication, lab, now)) {
        cancelOutbox(tx, ref, 'delivery-forbidden');
        return;
      }
